...
describe('procurement quote classifications',()=>{
  it('keeps the five agreed categories in display order',()=>{
    expect(procurementQuoteCategories.map(category=>category.value)).toEqual([
      'FOB_USD','FOB_CNY','ALL_IN_PORT_CNY','EX_FACTORY_CNY','REPROCESSING_CNY',
    ])
  })

  it('locks each category to its agreed currency',()=>{
    expect(procurementQuoteCurrency('FOB_USD')).toBe('USD')
    for(const category of procurementQuoteCategories.slice(1))expect(category.currency).toBe('CNY')
