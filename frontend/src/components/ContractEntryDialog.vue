...
async function searchCustomers(keyword:string){customers.value=(await get<{customers:any[]}>('/customers',{keyword,page_size:50})).customers||[]}
...
async function uploadFile(){if(!file.value)return;const f=file.value;const signed=await post<{fileKey:string;uploadUrl:string}>('/contracts/existing/files/presign',{fileName:f.name,contentType:f.type||'application/pdf'});const r=await fetch(signed.uploadUrl,{method:'PUT',headers:{'Content-Type':f.type||'application/pdf'},body:f});if(!r.ok)throw new Error('签署文件上传失败');form.signedFileKey=signed.fileKey;form.signedFileName=f.name;file.value=null}
async function loadDrafts(){drafts.value=await post('/contract-history-drafts',{action:'history_list'})}
function openDraft(id:string){const d=drafts.value.find(d=>d.id===id);if(!d)return;Object.assign(form,initial(),JSON.parse(JSON.stringify(d.body)));draftId.value=d.id;revision.value=d.revision;file.value=null;if(fileInput.value)fileInput.value.value='';if(form.customerId&&!customers.value.some(c=>String(c.id)===form.customerId))customers.value.push({id:form.customerId,name:form.customerName})}
async function persistDraft(){await uploadFile();const d=await post<{id:string;revision:number;contractId?:string}>('/contract-history-drafts',{action:'history_save',id:Number(draftId.value||0),revision:revision.value,data:form});draftId.value=d.id;revision.value=d.revision;return d.contractId}
async function saveHistoryDraft(){if(busy.value)return;busy.value=true;try{const linked=await persistDraft();if(linked){visible.value=false;emit('saved',linked);ElMessage.info('此草稿已接入，已打开原合同；未覆盖原合同资料');return}await loadDrafts();ElMessage.success('历史合同草稿已保存')}finally{busy.value=false}}
async function removeDraft(){await ElMessageBox.confirm('删除这份未接入的历史草稿？删除后无法恢复。','删除草稿',{type:'warning'});await post('/contract-history-drafts',{action:'history_delete',id:Number(draftId.value),revision:revision.value});Object.assign(form,initial());draftId.value='';revision.value=0;addLine();await loadDrafts()}
...
async function save(){
 ...
 try{
  ...
  if(props.history){const linked=await persistDraft();if(linked){visible.value=false;emit('saved',linked);ElMessage.info('此草稿已接入，已打开原合同');return}result=await post('/contracts/existing',{...payload(),items:[],terms:{terms:form.terms},signedDate:form.signedDate,effectiveDate:'',signedFileKey:form.signedFileKey,signedFileName:form.signedFileName,historyJson:JSON.stringify({recordOnly:true,totalAmount:form.totalAmount.trim(),status:form.historyStatus}),draftId:draftId.value,draftRevision:revision.value},withIdempotency(idem))}
  else {result=await post('/contracts/direct',payload(),withIdempotency(idem))}
  idem.reset();visible.value=false;emit('saved',result.contract.id);ElMessage.success(props.history?'历史合同已保存':'合同草稿已建立')
 }finally{busy.value=false}
}
watch(()=>props.modelValue,async v=>{importOpen.value=false;if(!v)return;Object.assign(form,initial());draftId.value='';revision.value=0;file.value=null;idem.reset();addLine();await searchCustomers('');if(props.history)await loadDrafts()})
</script>
<style scoped>
.entry-form{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:0 18px;margin-top:18px}.history-form{grid-template-columns:repeat(2,minmax(0,1fr))}.file-name{margin-left:12px;overflow-wrap:anywhere}.file-hint{flex-basis:100%;margin-top:6px}.entry-form :deep(.el-select),.entry-form :deep(.el-date-editor){width:100%}.lines-heading,.drafts{display:flex;align-items:center;gap:12px;margin:16px 0}.lines-heading{justify-content:space-between}.entry-total{text-align:right;font-weight:600;margin:14px 0}small{display:block;font-size:12px;color:#64748b;line-height:1.6}@media(max-width:760px){.entry-form{grid-template-columns:1fr 1fr}.drafts{flex-wrap:wrap}}@media(max-width:480px){.entry-form{grid-template-columns:1fr}}
</style>
