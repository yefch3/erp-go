<template>
  <el-dialog v-model="open" title="更新船期进度" width="620px" destroy-on-close>
    <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
      ...
      <el-form-item label="进度动作" prop="action"><el-select v-model="form.action" style="width:100%"><el-option label="正在驶向" value="APPROACH"/><el-option label="已到港" value="ARRIVE"/><el-option label="已离港" value="DEPART"/><el-option label="跳过此港" value="SKIP"/><el-option label="更新目的港ETA" value="UPDATE_ETA"/></el-select></el-form-item>
      <el-form-item v-if="['ARRIVE','DEPART'].includes(form.action)" label="实际时间"><el-date-picker v-model="form.actualTime" type="datetime" value-format="YYYY-MM-DDTHH:mm:ssZ" style="width:100%" /></el-form-item>
      <template v-if="form.action==='UPDATE_ETA'">
        <el-form-item label="最新ETA" prop="latestEta"><el-date-picker v-model="form.latestEta" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item>
      ...
      <el-form-item label="操作原因" prop="reason"><el-input v-model="form.reason" type="textarea" :rows="2" /></el-form-item>
...
const empty=()=>({routeNodeId:'',action:'APPROACH',actualTime:'',latestEta:'',reasonCode:'OTHER',reason:'',impactType:'SCHEDULE',affectedNodeId:'',fromNodeId:'',toNodeId:'',note:''});const form=reactive(empty())
const rules:FormRules={routeNodeId:[{required:true,message:'请选择港口'}],action:[{required:true}],latestEta:[{validator:(_:unknown,v:string,done:(e?:Error)=>void)=>form.action==='UPDATE_ETA'&&!v?done(new Error('请选择最新ETA')):done()}],reason:[{required:true,message:'请输入操作原因'}]}
watch(open,v=>{if(v)Object.assign(form,empty(),{routeNodeId:props.nodes[0]?.id??''})})
async function save(){if(!await formRef.value?.validate().catch(()=>false))return;if(form.action==='UPDATE_ETA'&&form.impactType==='LEG'&&(!form.fromNodeId||!form.toNodeId)){ElMessage.warning('请选择受影响航段');return}saving.value=true;try{await post(`/shipping/schedules/${props.scheduleId}/progress`,{...form,routeVersion:props.routeVersion,affectedNodeId:form.impactType==='PORT'?form.routeNodeId:'0',fromNodeId:form.impactType==='LEG'?form.fromNodeId:'0',toNodeId:form.impactType==='LEG'?form.toNodeId:'0'});ElMessage.success('船期进度已更新');open.value=false;emit('saved')}finally{saving.value=false}}
</script>
<style scoped>.leg-select{display:flex;align-items:center;gap:8px;width:100%}.leg-select .el-select{flex:1}</style>
