<template>
  <el-dialog v-model="open" :title="t('shipping.addRouteNode')" width="580px" destroy-on-close>
    <el-form ref="formRef" :model="form" :rules="rules" label-width="110px">
      ...
      <el-form-item :label="t('shipping.port')" prop="portId"><PortSelect v-model="form.portId" @selected="applyPort" /></el-form-item>
...
watch(open,v=>{if(!v)return;Object.assign(form,empty(),{insertAfterNodeId:insertOptions.value.at(-1)?.id??''})})
function portLabel(port:PortOption){return locale.value==='zh'?(port.nameZh||port.nameEn):(port.nameEn||port.nameZh)}
function applyPort(port:PortOption|undefined){if(port)Object.assign(form,{portName:portLabel(port),portCode:port.unlocode,timezone:port.timezone})}
