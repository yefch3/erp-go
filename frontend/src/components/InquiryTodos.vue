<template>
  <section v-if="rows.length || error || mode==='handled'" class="inquiry-todos">
    <div class="section-head">
      <div><h3>{{mode==='handled'?'询盘与报价处理记录':'询盘与报价'}}</h3><p>{{mode==='handled'?'查看询盘、报价与合同流程中已经完成的业务动作。':'需要你处理的客户询盘、采购报价和物流报价。'}}</p></div>
