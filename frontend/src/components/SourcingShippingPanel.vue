  ...
  <section class="presales-workspace">
    ...
    <el-dialog v-model="planOpen" :title="t('presalesShipping.createManagerPlan')" width="min(1240px, 96vw)">
      ...
      <div class="table-scroll"><el-table :data="planRows" border style="min-width:1120px">
        ...
        <el-table-column :label="t('presalesShipping.selectionType')" width="130"><template #default="{ row }"><el-select v-model="row.selectionType" :disabled="!row.selected"><el-option value="RECOMMENDED" :label="t('presalesShipping.recommended')"/><el-option value="BACKUP" :label="t('presalesShipping.backup')"/><el-option value="REJECTED" :label="t('presalesShipping.rejected')"/></el-select></template></el-table-column>
...
async function createPlan() {
  ...
  if (selected.some((row: any) => !String(row.reason).trim())) { ElMessage.warning(t('presalesShipping.decisionReasonRequired')); return }
