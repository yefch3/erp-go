  ...
  <div class="page">
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑仓库' : '新建仓库'" width="760px" destroy-on-close>
      <el-form label-width="110px" class="form-grid">
        ...
        <el-form-item label="仓库编码" required><el-input v-model="form.code" placeholder="例如 WH-SH-01"/></el-form-item>
        ...
        <el-form-item label="国家/地区"><el-input v-model="form.countryCode" placeholder="ISO 代码，例如 CN"/></el-form-item>
        <el-form-item label="城市"><el-input v-model="form.city"/></el-form-item>
        <el-form-item label="IANA 时区"><el-select v-model="form.timezone" filterable allow-create><el-option v-for="tz in timezones" :key="tz" :label="tz" :value="tz"/></el-select></el-form-item>
        ...
        <el-form-item label="变更原因" class="full"><el-input v-model="form.changeReason" placeholder="停用或重要修改时请填写"/></el-form-item>
      </el-form>
      <div class="contact-head"><strong>联系人与内部负责人</strong><el-button link type="primary" @click="addContact">添加联系人</el-button></div>
      <div v-for="(contact,index) in form.contacts" :key="index" class="contact-row">
        <el-select v-model="contact.contactType"><el-option label="内部负责人" value="OWNER"/><el-option label="外部联系人" value="CONTACT"/></el-select>
        <el-input v-model="contact.name" placeholder="姓名"/><el-input v-model="contact.phone" placeholder="电话"/><el-input v-model="contact.email" placeholder="邮箱"/>
...
const router=useRouter(), loading=ref(false), saving=ref(false), dialogVisible=ref(false), editingId=ref(''), keyword=ref(''), profileFilter=ref(''), warehouses=ref<Warehouse[]>([])
const profileOptions=[{label:'公司自有仓',value:'OWN'},{label:'港口仓库',value:'PORT'},{label:'第三方仓库',value:'THIRD_PARTY'}]
const timezones=['UTC','Asia/Shanghai','Asia/Hong_Kong','Asia/Singapore','Asia/Tokyo','Europe/London','America/New_York','America/Los_Angeles']
const blank=()=>({code:'',name:'',profileType:'OWN',address:'',countryCode:'',city:'',timezone:'UTC',accountingMode:'SYNC_INVENTORY',status:'ACTIVE',changeReason:'',contacts:[] as Contact[]})
...
function openEdit(row:Warehouse){editingId.value=row.id;reset({...blank(),...row,changeReason:'',contacts:(row.contacts??[]).map(x=>({...x}))});dialogVisible.value=true}
...
async function save(){saving.value=true;try{const body={warehouse:{...form}};if(editingId.value)await put(`/warehouses/${editingId.value}`,body);else await post('/warehouses',body);ElMessage.success('仓库档案已保存');dialogVisible.value=false;await load()}finally{saving.value=false}}
onMounted(load)
</script>

<style scoped>
.page{padding:28px;max-width:1500px;margin:auto}.hero{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}.hero h1{margin:3px 0;font-size:30px}.hero p{margin:4px 0;color:#748196}.eyebrow{font-size:12px!important;letter-spacing:2px;color:#087f78!important;font-weight:700}.filters{display:flex;gap:12px;margin-bottom:16px}.filters .el-input{max-width:360px}.filters .el-select{width:180px}small{display:block;color:#8a96a6;margin-top:4px}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:0 14px}.form-grid .full{grid-column:1/-1}.contact-head{display:flex;justify-content:space-between;border-top:1px solid #e5eaf0;padding:15px 0 10px}.contact-row{display:grid;grid-template-columns:130px 1fr 1fr 1.2fr auto auto;gap:8px;align-items:center;margin-bottom:8px}@media(max-width:800px){.hero{align-items:flex-start;flex-direction:column;gap:12px}.form-grid{grid-template-columns:1fr}.form-grid .full{grid-column:auto}.contact-row{grid-template-columns:1fr 1fr}.page{padding:16px}}
</style>
