<template>
  <div class="page">
    <header class="hero">
      <div><p class="eyebrow">{{ t('warehouse.receipts.eyebrow') }}</p><h1>{{ t('warehouse.receipts.title') }}</h1><p>{{ t('warehouse.receipts.subtitle') }}</p></div>
      <div><el-button @click="router.push('/warehouses/arrivals')">{{ t('warehouse.receipts.arrivals') }}</el-button><el-button @click="router.push('/warehouses')">{{ t('warehouse.backWorkbench') }}</el-button></div>
    </header>
    <el-card shadow="never" class="filters">
      <el-input v-model="keyword" clearable :placeholder="t('warehouse.receipts.search')" @keyup.enter="applyFilters" @clear="applyFilters" />
      <el-select v-model="deliveryType" @change="applyFilters"><el-option :label="t('warehouse.receipts.allTypes')" value="ALL"/><el-option :label="t('warehouse.receipts.direct')" value="DIRECT"/><el-option :label="t('warehouse.receipts.warehouse')" value="WAREHOUSE"/></el-select>
    ...
    <el-card shadow="never">
      <el-table :data="pagedRows" v-loading="loading" row-key="id">
        <el-table-column :label="t('warehouse.receipts.receiptNo')" min-width="170"><template #default="{row}"><strong>{{row.receiptNo}}</strong><div class="sub">{{formatTime(row.receivedAt)}}</div></template></el-table-column>
        <el-table-column :label="t('warehouse.receipts.purchaseOrder')" min-width="150" prop="poNo"/>
        <el-table-column :label="t('warehouse.receipts.supplier')" min-width="180" prop="supplierName"/>
        <el-table-column :label="t('warehouse.receipts.typeLocation')" min-width="220"><template #default="{row}"><el-tag size="small" :type="isDirectReceipt(row)?'info':'success'">{{isDirectReceipt(row)?t('warehouse.receipts.direct'):t('warehouse.receipts.warehouse')}}</el-tag><div class="sub destination">{{row.destination}}</div></template></el-table-column>
        <el-table-column :label="t('warehouse.receipts.quantity')" width="120" align="right"><template #default="{row}"><strong>{{trim(row.totalQty)}}</strong></template></el-table-column>
        <el-table-column :label="t('warehouse.receipts.operator')" width="130"><template #default="{row}">{{row.operatorName||'—'}}</template></el-table-column>
        <el-table-column :label="t('warehouse.receipts.remark')" min-width="160"><template #default="{row}">{{row.remark||'—'}}</template></el-table-column>
        <template #empty><el-empty :description="t('warehouse.receipts.empty')"/></template>
      </el-table>
...
function destination(order:Order,receipt:Receipt){if(!isDirectReceipt(receipt))return warehouseNames.value[String(receipt.warehouseId)]||order.warehouseName||t('warehouse.receipts.warehouseNumber',{id:receipt.warehouseId});if(order.deliveryLocationType==='PORT')return [order.deliveryPortCode,order.deliveryPortName].filter(Boolean).join(' · ')||t('warehouse.receipts.port');return order.deliveryAddress||t('warehouse.receipts.specifiedLocation')}
