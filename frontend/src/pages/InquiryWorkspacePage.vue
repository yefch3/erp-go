<template>
 <main class="inquiry-workspace" :class="{ 'inquiry-workspace--procurement': view === 'PROCUREMENT' }" v-loading="busy">
  ...
  <template v-else>
   ...
   <section v-if="editor" class="quote-editor"><div class="quote-editor-heading"><div><h3>{{view==='PROCUREMENT'?'工厂报价':'货代报价'}}</h3><p>录入收到的原始报价、币种和贸易条款；提交后由销售结合上下游数据计算客户单价。</p></div><el-tag effect="plain">原始报价</el-tag></div><el-form label-position="top" class="inquiry-fields"><template v-if="view==='PROCUREMENT'"><el-form-item label="工厂或供应商"><el-autocomplete v-model="editor.body.company" :fetch-suggestions="companySuggestions" placeholder="选择已有数据或直接输入" :trigger-on-focus="true"/></el-form-item><el-form-item label="工厂报价币种"><el-input v-model="editor.body.currency" maxlength="3"/></el-form-item><el-form-item label="报价贸易条款"><el-input v-model="editor.body.incoterm" placeholder="例如 FOB、CFR、CIF"/></el-form-item><el-form-item label="统一预计交货日期"><el-date-picker v-model="editor.body.delivery" type="date" value-format="YYYY-MM-DD"/></el-form-item></template><template v-else><el-form-item v-for="f in logisticsPrimaryFields" :key="f.key" :label="f.label"><el-input v-model="editor.body[f.key]"/></el-form-item><el-form-item label="报价贸易条款"><el-input v-model="editor.body.incoterm" placeholder="例如 FOB、CFR、CIF"/></el-form-item><el-form-item label="预计开船日期"><el-date-picker v-model="editor.body.departure" value-format="YYYY-MM-DD"/></el-form-item><el-form-item label="预计到港日期"><el-date-picker v-model="editor.body.arrival" value-format="YYYY-MM-DD"/></el-form-item></template></el-form>
    <el-collapse><el-collapse-item title="更多信息" name="more"><el-form label-position="top" class="inquiry-fields"><template v-if="view==='LOGISTICS'"><el-form-item label="签约货代公司"><el-autocomplete v-model="editor.body.company" :fetch-suggestions="companySuggestions" placeholder="按正式需求需要时填写" :trigger-on-focus="true"/></el-form-item><el-form-item label="方案名称"><el-input v-model="editor.body.route" placeholder="按正式需求需要区分多方案时填写"/></el-form-item></template><el-form-item label="报价有效期"><el-date-picker v-model="editor.body.validUntil" value-format="YYYY-MM-DD"/></el-form-item><el-form-item label="付款方式"><el-input v-model="editor.body.paymentTerms"/></el-form-item><el-form-item label="报价附件" class="wide"><input type="file" @change="upload($event,true)"/><el-tag v-for="a in editor.body.attachments" :key="a.key" closable @close="editor!.body.attachments=editor!.body.attachments.filter(x=>x.key!==a.key)">{{a.name}}</el-tag></el-form-item><el-form-item label="备注" class="wide"><el-input v-model="editor.body.remark" type="textarea"/></el-form-item></el-form></el-collapse-item></el-collapse>
     ...
     <el-tab-pane v-if="view==='QUOTATIONS'" label="给客户的报价" name="offer"><CustomerOfferEditor :case-id="item.id" :source-version="item.quotes.map(q=>`${q.id}:${q.version}`).join(',')" /></el-tab-pane>
    </el-tabs>
   </template>
...
const quoteDisplayFields=[{key:'company',label:'工厂 / 货代'},{key:'currency',label:'币种'},{key:'incoterm',label:'报价贸易条款'},{key:'delivery',label:'统一交货日期'},{key:'validUntil',label:'报价有效期'},{key:'paymentTerms',label:'付款方式'},...logisticsFields,{key:'departure',label:'开船日期'},{key:'arrival',label:'到港日期'},{key:'remark',label:'备注'}] as const
...
.inquiry-workspace--procurement .inquiry-list-panel :deep(.el-card__body) { padding:0; }
.inquiry-workspace--procurement .list-toolbar { margin:0; padding:16px 18px; border:0; border-bottom:1px solid #e2eef3; border-radius:0; background:linear-gradient(90deg,#f8fcfe 0%,#fff 72%); box-shadow:none; }
.inquiry-workspace--procurement .list-toolbar :deep(.el-input__wrapper),
.inquiry-workspace--procurement .list-toolbar :deep(.el-select__wrapper) { background:#fff; box-shadow:0 0 0 1px #d6e8f0 inset; }
.list-summary { display:flex; align-items:baseline; gap:5px; margin-left:auto; padding:6px 10px; color:#657782; background:#eefaff; border:1px solid #c9edff; border-radius:999px; font-size:12px; white-space:nowrap; }
.list-summary strong { color:#108fc9; font-size:15px; font-variant-numeric:tabular-nums; }
.inquiry-workspace--procurement :deep(.inquiry-list-table) { margin:0; border:0; border-radius:0; --el-table-header-bg-color:#eaf8ff; --el-table-header-text-color:#263943; --el-table-row-hover-bg-color:#effbf5; }
.inquiry-workspace--procurement :deep(.inquiry-list-table th.el-table__cell) { height:50px; border-bottom-color:#d7edf6; font-weight:650; }
.inquiry-workspace--procurement :deep(.inquiry-list-table td.el-table__cell) { padding:15px 0; border-bottom-color:#e7eff3; color:#253b49; }
.inquiry-workspace--procurement :deep(.inquiry-list-table .el-table__row--striped td.el-table__cell) { background:#fbfdfe; }
.inquiry-workspace--procurement :deep(.inquiry-list-table .el-button--primary.is-plain) { color:#087fb6; border-color:#8ed8fb; background:#f0faff; font-weight:600; }
...
.inquiry-workspace--procurement .list-pagination { justify-content:flex-end; margin:0; padding:16px 18px; background:#fff; }
