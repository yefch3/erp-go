<template>
 <main class="inquiry-workspace inquiry-workspace--operations" v-loading="busy">
  <WorkflowPageHeader v-if="!item" :title="title" :description="subtitle">
   <template v-if="view==='SALES'" #actions><div class="heading-actions"><el-button @click="router.push('/sales/settings/inquiry-templates')">询盘模板</el-button><el-button class="mail-import-action" @click="router.push('/emails')">从邮箱转入询盘</el-button></div></template>
  </WorkflowPageHeader>
  <el-card v-if="!item" class="inquiry-list-panel" shadow="never">
   <div class="toolbar list-toolbar"><el-input v-model="keyword" :placeholder="department?'搜索询盘编号、客户、产品或规格':'搜索编号、客户、产品或规格'" clearable @change="loadList"/><el-select v-model="state" clearable placeholder="全部状态" @change="loadList"><el-option v-for="o in states" :key="o.value" :value="o.value" :label="o.label"/></el-select><el-button @click="loadList">刷新</el-button><el-button v-if="view==='SALES'&&canWrite" class="create-inquiry-action" type="primary" @click="openCreateDialog">上传或新建客户询盘</el-button></div>
   <el-table class="inquiry-list-table" :data="items" stripe @row-dblclick="open"><el-table-column label="询盘编号" min-width="190"><template #default="{row}"><span class="inquiry-number">{{row.number}}</span></template></el-table-column><el-table-column prop="body.customer" label="客户" min-width="175" show-overflow-tooltip/><el-table-column prop="owner" label="负责销售" min-width="105"/><el-table-column label="产品项数" min-width="95"><template #default="{row}">{{row.body.products?.length||0}}</template></el-table-column><el-table-column v-if="department" label="总需求数量" min-width="120"><template #default="{row}">{{productTotal(row.body.products,'quantity','unit')}}</template></el-table-column><el-table-column v-if="view==='LOGISTICS'" label="总重量 / 总体积" min-width="180"><template #default="{row}">{{productTotal(row.body.products,'weight')}} / {{productTotal(row.body.products,'volume')}}</template></el-table-column><el-table-column v-if="department" prop="body.delivery" label="交货要求" min-width="120"/><el-table-column v-if="view==='LOGISTICS'" prop="body.loadingPort" label="装货港" min-width="120"/><el-table-column v-if="view==='LOGISTICS'" prop="body.destinationPort" label="目的港" min-width="120"/><el-table-column v-if="view==='LOGISTICS'" prop="body.incoterm" label="贸易条件" min-width="100"/><el-table-column v-if="department" :label="view==='PROCUREMENT'?'已提交工厂报价':'已提交物流报价'" min-width="135"><template #default="{row}">{{view==='PROCUREMENT'?row.procurementCount:row.logisticsCount}}</template></el-table-column><el-table-column label="提交时间" min-width="160"><template #default="{row}">{{displayTime(row.submittedAt)}}</template></el-table-column><el-table-column label="状态" min-width="95"><template #default="{row}"><el-tag effect="light" :type="row.state==='WITHDRAWN'?'info':row.state==='INQUIRING'?'success':'warning'">{{view==='QUOTATIONS'?offerStateLabel(row.id):stateLabel(row)}}</el-tag></template></el-table-column><el-table-column label="操作" min-width="145" align="center"><template #default="{row}"><el-button :link="!department" :plain="department" type="primary" @click="open(row)">{{view==='PROCUREMENT'?(row.procurementCount>0?'查看 / 继续报价':'录入工厂报价'):department?(row.logisticsCount>0?'查看 / 继续报价':'录入物流报价'):'打开详情 →'}}</el-button></template></el-table-column></el-table>
