  ...
  <template v-else>
   <section class="detail-hero">
    ...
    <el-tabs v-model="detailTab" class="detail-tabs">
     ...
     <section class="business-card customer-card"><div class="section-title"><div><h3>{{t('inquiryWorkspace.detail.customerDelivery')}}</h3><p>{{t('inquiryWorkspace.detail.customerDeliveryHint')}}</p></div></div><el-form label-position="top" :disabled="!editable" class="inquiry-fields"><el-form-item :label="t('inquiryWorkspace.customer')"><el-autocomplete v-if="editable" v-model="item.body.customer" :fetch-suggestions="customerSuggestions" :placeholder="t('inquiryWorkspace.detail.customerSearch')" clearable @select="selectCustomer" @input="customerTyped"><template #default="{item:option}"><div class="option-title">{{option.name}}</div><span class="option-meta">{{option.code}}</span></template></el-autocomplete><el-input v-else v-model="item.body.customer" placeholder="—"/></el-form-item><el-form-item :label="t('inquiryWorkspace.detail.contact')"><el-autocomplete v-if="editable" v-model="item.body.contact" :fetch-suggestions="contactSuggestions" :placeholder="t('inquiryWorkspace.detail.contactSearch')" clearable @select="selectContact" @input="contactTyped"><template #default="{item:option}"><div class="option-title">{{option.name}}</div><span class="option-meta">{{contactOptionLabel(option)}}</span></template></el-autocomplete><el-input v-else v-model="item.body.contact" placeholder="—"/></el-form-item><el-form-item :label="t('inquiryWorkspace.delivery')"><el-input v-model="item.body.delivery" placeholder="—"/></el-form-item><el-form-item :label="t('inquiryWorkspace.loadingPort')"><el-input v-model="item.body.loadingPort" placeholder="—"/></el-form-item><el-form-item :label="t('inquiryWorkspace.destinationPort')"><el-input v-model="item.body.destinationPort" placeholder="—"/></el-form-item><el-form-item :label="t('inquiryWorkspace.incoterm')"><el-input v-model="item.body.incoterm" placeholder="—"/></el-form-item><el-form-item :label="t('inquiryWorkspace.detail.remark')" class="wide"><el-input v-model="item.body.remark" type="textarea" :rows="2" placeholder="—"/></el-form-item></el-form></section>
     <section class="business-card attachment-card"><div class="attachment-heading"><div><h3>{{t('inquiryWorkspace.detail.attachments')}}</h3><p>{{item.body.attachments.length?t('inquiryWorkspace.detail.attachmentCount',{count:item.body.attachments.length}):t('inquiryWorkspace.detail.noAttachments')}}</p></div><input v-if="editable" class="file-input" type="file" @change="upload($event,false)"/></div><div v-if="item.body.attachments.length" class="attachment-list"><el-button v-for="a in item.body.attachments" :key="a.key" plain @click="download(a.key)">📎 {{a.name}}</el-button></div><div v-else class="attachment-empty">{{t('inquiryWorkspace.detail.attachmentEmpty')}}</div></section>
     </div></el-tab-pane>
     <el-tab-pane :label="t('inquiryWorkspace.detail.productRequirements',{count:item.body.products.length})" name="products"><section class="business-card product-card"><div class="section-title"><div><h3>{{t('inquiryWorkspace.detail.requirementsTitle')}}</h3><p>{{t('inquiryWorkspace.detail.requirementsHint')}}</p></div><div class="template-actions"><template v-if="view==='SALES'&&editable"><span class="template-label">{{t('inquiryWorkspace.templates')}}</span><el-select :model-value="item.body.template?.id" :placeholder="t('inquiryWorkspace.detail.selectTemplate')" @change="changeTemplate"><el-option v-for="template in selectableTemplates" :key="template.id" :label="`${template.name} · v${template.version}${template.status==='ACTIVE'?'':t('inquiryWorkspace.detail.historical')}`" :value="template.id"/></el-select><el-button type="primary" plain @click="addProduct">{{t('inquiryWorkspace.detail.addProduct')}}</el-button></template><el-tag v-else-if="item.body.template" effect="plain">{{item.body.template.name}} · v{{item.body.template.version}}</el-tag></div></div><el-collapse v-if="editor" v-model="requirementsOpen"><el-collapse-item :title="t('inquiryWorkspace.detail.viewRequirements')" name="products"><InquiryProducts :products="item.body.products" :fields="templateFields"/></el-collapse-item></el-collapse><InquiryProducts v-else :products="item.body.products" :fields="templateFields" :editable="!!editable" @remove="item.body.products.splice($event,1)"/></section></el-tab-pane>
     <el-tab-pane v-if="view!=='SALES'" :label="t('inquiryWorkspace.detail.submittedQuotes',{count:item.quotes?.length||0})" name="quotes">
     <section v-if="!editor" class="business-card quotes-section">
      <div class="section-title">
       <div><h3>{{view==='QUOTATIONS'?'报价对比':view==='PROCUREMENT'?'工厂报价':'货代报价'}}</h3><p>一行一份报价；提交后供销售选择并按公司公式计算。</p></div>
       <el-tag effect="plain">{{item.quotes?.length||0}} 份报价</el-tag>
      </div>
      <el-table :data="item.quotes||[]" :max-height="500" stripe empty-text="暂无报价">
       <el-table-column prop="body.company" label="报价方" min-width="180" show-overflow-tooltip/>
       <el-table-column label="类型" width="100"><template #default="{row:q}"><el-tag size="small" :type="q.kind==='PROCUREMENT'?'warning':'success'">{{q.kind==='PROCUREMENT'?'工厂':'货代'}}</el-tag></template></el-table-column>
       <el-table-column label="报价内容" min-width="250"><template #default="{row:q}"><strong class="compact-quote">{{q.kind==='PROCUREMENT'?`${q.body.prices?.length||0}/${item.body.products.length} 项 · ${q.body.currency||'—'}`:quoteTotals(q)||'等待报价'}}</strong><small>{{q.kind==='PROCUREMENT'?`交期：${q.body.delivery||'按产品'}`:`${q.body.carrier||'船公司未填'} · ${q.body.route||'方案未填'}`}}</small></template></el-table-column>
       <el-table-column label="报价人 / 修改人" min-width="180"><template #default="{row:q}">{{q.author||'—'}}<small>{{q.updatedBy&&q.updatedBy!==q.author?`修改：${q.updatedBy}`:''}}</small></template></el-table-column>
       <el-table-column label="提交时间" min-width="155"><template #default="{row:q}">{{displayTime(q.submittedAt)}}</template></el-table-column>
       <el-table-column label="状态" width="100"><template #default="{row:q}"><el-tag size="small" effect="plain" :type="q.historical?'info':q.submittedAt?'success':'warning'">{{q.historical?'历史版本':q.submittedAt?'已提交':'已保存'}}</el-tag></template></el-table-column>
       <el-table-column label="操作" width="180" fixed="right"><template #default="{row:q}"><el-button v-if="q.canEdit" link type="primary" @click="editQuote(q)">修改</el-button><el-button link type="primary" @click="preview=q">查看完整报价</el-button></template></el-table-column>
      </el-table>
     </section>
   <section v-if="editor" class="quote-editor"><div class="quote-editor-heading"><div><h3>{{view==='PROCUREMENT'?'工厂报价':'货代报价'}}</h3><p>填写报价、币种和计价单位，贸易条件由销售确定。</p></div><el-tag effect="plain">原始报价</el-tag></div><el-form label-position="top" class="inquiry-fields"><template v-if="view==='PROCUREMENT'"><el-form-item label="工厂或供应商"><el-autocomplete v-model="editor.body.company" :fetch-suggestions="companySuggestions" placeholder="选择已有数据或直接输入" :trigger-on-focus="true"/></el-form-item><el-form-item label="工厂报价币种"><el-input v-model="editor.body.currency" maxlength="3"/></el-form-item><el-form-item label="统一预计交货日期"><el-date-picker v-model="editor.body.delivery" type="date" value-format="YYYY-MM-DD"/></el-form-item></template><template v-else><el-form-item label="货代名称"><el-input v-model="editor.body.company"/></el-form-item><el-form-item label="报价币种"><el-input v-model="editor.body.currency" maxlength="3"/></el-form-item><el-form-item label="计价单位"><el-input v-model="logisticsUnit" placeholder="例如 MT、件、整票"/></el-form-item><el-form-item v-for="f in logisticsPrimaryFields" :key="f.key" :label="f.label"><el-input v-model="editor.body[f.key]"/></el-form-item><el-form-item label="预计开船日期"><el-date-picker v-model="editor.body.departure" value-format="YYYY-MM-DD"/></el-form-item><el-form-item label="预计到港日期"><el-date-picker v-model="editor.body.arrival" value-format="YYYY-MM-DD"/></el-form-item></template></el-form>
    <el-collapse><el-collapse-item title="更多信息" name="more"><el-form label-position="top" class="inquiry-fields"><template v-if="view==='LOGISTICS'"><el-form-item label="签约货代公司"><el-autocomplete v-model="editor.body.company" :fetch-suggestions="companySuggestions" placeholder="按正式需求需要时填写" :trigger-on-focus="true"/></el-form-item><el-form-item label="方案名称"><el-input v-model="editor.body.route" placeholder="按正式需求需要区分多方案时填写"/></el-form-item></template><el-form-item label="报价有效期"><el-date-picker v-model="editor.body.validUntil" value-format="YYYY-MM-DD"/></el-form-item><el-form-item label="付款方式"><el-input v-model="editor.body.paymentTerms"/></el-form-item><el-form-item label="报价附件" class="wide"><input type="file" @change="upload($event,true)"/><el-tag v-for="a in editor.body.attachments" :key="a.key" closable @close="editor!.body.attachments=editor!.body.attachments.filter(x=>x.key!==a.key)">{{a.name}}</el-tag></el-form-item><el-form-item label="备注" class="wide"><el-input v-model="editor.body.remark" type="textarea"/></el-form-item></el-form></el-collapse-item></el-collapse>
     <template v-if="view==='PROCUREMENT'"><div class="toolbar"><el-input v-model="productSearch" placeholder="搜索产品或规格" @input="productPage=1"/><el-select v-model="productFilter" @change="productPage=1"><el-option label="全部产品" value="all"/><el-option label="未报价产品" value="unquoted"/><el-option label="已报价产品" value="quoted"/></el-select><span>已报价 {{editor.body.prices.length}} / {{item.body.products.length}} 项</span></div><el-table :data="pagedProducts" row-key="id" :max-height="520"><el-table-column label="选择" width="65"><template #default="{row}"><el-checkbox :model-value="!!priceFor(row.id)" @change="toggleProduct(row.id,!!$event)"/></template></el-table-column><el-table-column prop="product" label="产品" min-width="180"/><el-table-column prop="specification" label="规格" min-width="190"/><el-table-column prop="quantity" label="需求数量" width="100"/><el-table-column prop="unit" label="单位" width="75"/><el-table-column label="工厂价" min-width="130"><template #default="{row}"><el-input v-if="priceFor(row.id)" v-model="priceFor(row.id)!.price"/></template></el-table-column><el-table-column label="交货日期" min-width="180"><template #default="{row}"><el-date-picker v-if="priceFor(row.id)" v-model="priceFor(row.id)!.delivery" value-format="YYYY-MM-DD" :placeholder="editor.body.delivery||'交货日期'"/></template></el-table-column><el-table-column label="备注" min-width="170"><template #default="{row}"><el-input v-if="priceFor(row.id)" v-model="priceFor(row.id)!.remark"/></template></el-table-column></el-table><p>每个产品填写一个工厂单价，计价单位与本行产品单位一致。</p><el-pagination v-model:current-page="productPage" v-model:page-size="productSize" :page-sizes="[20,50,100]" :total="filteredProducts.length" layout="total, sizes, prev, pager, next"/></template>
     <template v-else><h4>适用货物（不勾选时按整批）</h4><InquiryProducts :products="item.body.products" :fields="templateFields" v-model:cargo-ids="editor.body.cargoIds"/><h4>费用构成</h4><p>各项费用请先折算为上方币种及同一计价单位，合计为一份物流报价。</p><el-table :data="editor.body.charges" :max-height="420"><el-table-column v-for="f in chargeFields" :key="f.key" :label="f.label" :min-width="f.width"><template #default="{row}"><span v-if="f.key==='subtotal'">{{chargeSubtotal(row.amount,row.quantity)||'—'}}</span><el-select v-else-if="f.key==='allocationType'" v-model="row.allocationType"><el-option value="DIRECT" label="产品专属"/><el-option value="PER_TON" label="按吨"/><el-option value="FIXED" label="整票固定"/></el-select><el-select v-else-if="f.key==='productId'" v-model="row.productId" :disabled="row.allocationType!=='DIRECT'" clearable><el-option v-for="p in item.body.products" :key="p.id" :value="p.id" :label="p.product"/></el-select><el-input v-else v-model="row[f.key]"/></template></el-table-column><el-table-column width="70"><template #default="{$index}"><el-button link type="danger" @click="editor!.body.charges.splice($index,1)">移除</el-button></template></el-table-column></el-table><el-button @click="addCharge">添加费用</el-button><p v-for="(v,k) in chargeTotals(editor.body.charges)" :key="k">{{k}} 合计：{{v}}</p></template>
     ...
     <el-tab-pane v-if="view==='QUOTATIONS'" :label="t('inquiryWorkspace.detail.customerOffer')" name="offer"><CustomerOfferEditor :case-id="item.id" :source-version="item.quotes.map(q=>`${q.id}:${q.version}`).join(',')" /></el-tab-pane>
    </el-tabs>
   </template>
...
const productFields=[{key:'product',label:'产品',width:180},{key:'specification',label:'规格',width:190},{key:'delivery',label:'交货要求',width:150},{key:'weight',label:'重量',width:100},{key:'volume',label:'体积',width:100},{key:'packaging',label:'包装',width:130},{key:'remark',label:'备注',width:190}]
 const logisticsPrimaryFields=[{key:'carrier',label:'实际船公司'},{key:'vessel',label:'船名'},{key:'voyage',label:'航次'},{key:'loadingPort',label:'装货港'},{key:'destinationPort',label:'目的港'},{key:'transitDays',label:'预计航程'}] as const
 const logisticsFields=[...logisticsPrimaryFields,{key:'route',label:'方案名称'}] as const
...
const logisticsUnit=computed({get:()=>editor.value?.body.charges[0]?.unit||'MT',set:(value:string)=>{if(editor.value){if(!editor.value.body.charges.length)addCharge();editor.value.body.charges.forEach(c=>c.unit=value)}}})
const chargeFields=[{key:'name',label:'费用名称',width:140},{key:'amount',label:'金额',width:110},{key:'remark',label:'备注',width:170}]
...
function addCharge(){if(!editor.value)return;const unit=editor.value.body.charges[0]?.unit||'MT';editor.value.body.charges.push({name:'',amount:'',currency:editor.value.body.currency,unit,quantity:'1',subtotal:'',remark:'',allocationType:unit==='MT'?'PER_TON':'FIXED',productId:''})}
async function saveQuote(submit:boolean){if(!item.value||!editor.value)return;if(view.value==='LOGISTICS'){editor.value.body.charges.forEach(c=>{c.currency=editor.value!.body.currency.toUpperCase();c.unit=logisticsUnit.value;c.quantity='1';c.productId='';c.allocationType=c.unit==='MT'?'PER_TON':'FIXED'})}busy.value=true;try{const r=await command('quote',{id:item.value.id,revision:item.value.revision,quoteId:editor.value.id,quoteVersion:editor.value.version,submit,quote:editor.value.body});item.value=normalizeInquiry(r.item);editor.value=null;ElMessage.success(submit?'报价已提交，销售可以查看并计算':'报价已保存')}finally{busy.value=false}}
