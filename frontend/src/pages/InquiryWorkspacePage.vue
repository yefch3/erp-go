   ...
   <div class="inquiry-table-scroll"><el-table class="inquiry-list-table" :class="{'inquiry-list-table--sales':view==='SALES','inquiry-list-table--hierarchy':view!=='LOGISTICS'}" :data="items" row-key="id" stripe :expand-row-keys="expandedInquiryIds" @row-dblclick="openFromRow">
    ...
    <el-table-column v-for="column in listColumnOrder.columns.value" :key="column.key" :min-width="column.minWidth" :width="column.width" :align="column.align" :class-name="column.className" :label-class-name="column.className" :show-overflow-tooltip="column.showOverflowTooltip">
     ...
     <template #default="{row}">
      ...
      <span v-else-if="column.key==='customer'">{{row.body.customer||'—'}}</span>
      ...
      <span v-else-if="column.key==='totalQuantity'">{{productTotal(row.body.products,'quantity','unit')}}</span>
