<template>
 <main class="inquiry-workspace inquiry-workspace--operations" v-loading="busy">
  ...
  <el-card v-if="!item" class="inquiry-list-panel" shadow="never">
   ...
   <el-table v-if="view==='SALES'" class="inquiry-list-table inquiry-list-table--sales" :data="items" row-key="id" stripe :expand-row-keys="expandedInquiryIds" @row-dblclick="openFromRow">
    ...
    <el-table-column :label="t('inquiryWorkspace.list.customerShortName')" min-width="150" show-overflow-tooltip><template #default="{row}">{{customerShortName(row)}}</template></el-table-column>
    ...
    <el-table-column :label="t('common.status')" min-width="120"><template #default="{row}"><el-tag effect="light" :type="inquiryListStatus(row).type">{{inquiryListStatus(row).label}}</el-tag></template></el-table-column>
    <el-table-column :label="t('common.actions')" min-width="135" align="center"><template #default="{row}"><el-dropdown trigger="click" placement="bottom-end" @command="handleListAction($event,row)"><el-button class="list-more-actions">{{t('inquiryWorkspace.list.moreActions')}}<span class="more-arrow">▼</span></el-button><template #dropdown><el-dropdown-menu><el-dropdown-item command="view">{{t('inquiryWorkspace.list.view')}}</el-dropdown-item><el-dropdown-item v-if="auth.can('sales:inquiry:delete')&&row.canDelete" command="delete" divided class="danger-menu-item">{{t('inquiryWorkspace.list.delete')}}</el-dropdown-item></el-dropdown-menu></template></el-dropdown></template></el-table-column>
   </el-table>
   <el-table v-else-if="view==='QUOTATIONS'||view==='PROCUREMENT'" class="inquiry-list-table inquiry-list-table--hierarchy" :data="items" row-key="id" stripe :expand-row-keys="expandedInquiryIds" @row-dblclick="openFromRow">
    <el-table-column type="expand" width="1" class-name="hidden-native-expander">
     <template #default="{row}">
      <div class="hierarchy-context"><span><b>{{t('inquiryWorkspace.customer')}}</b>{{row.body.customer||'—'}}</span><span><b>{{t('inquiryWorkspace.owner')}}</b>{{row.owner||'—'}}</span></div>
    ...
    <el-table-column :label="t('inquiryWorkspace.number')" min-width="130"><template #default="{row}"><span class="inquiry-number">{{displayInquiryNumber(row)}}</span></template></el-table-column>
    <el-table-column v-if="view==='QUOTATIONS'" :label="t('inquiryWorkspace.list.customerShortName')" min-width="145" show-overflow-tooltip><template #default="{row}">{{customerShortName(row)}}</template></el-table-column>
    <el-table-column :label="t('inquiryWorkspace.list.productDetails')" min-width="220"><template #default="{row}"><button type="button" class="product-summary product-summary--stacked" :aria-expanded="isInquiryExpanded(row.id)" @click.stop="toggleInquiryProducts(row)"><span class="product-summary__main"><span class="product-summary__text" :title="inquiryProductSummary(row)">{{inquiryProductSummary(row)}}</span><span v-if="inquiryProductGroups(row).length>1" class="product-summary__count">+{{inquiryProductGroups(row).length-1}}</span><el-icon class="product-summary__chevron" :class="{'is-expanded':isInquiryExpanded(row.id)}"><ArrowDown/></el-icon></span><small class="product-summary__mobile-meta">{{view==='QUOTATIONS'?`采购 ${row.procurementCount} 份 · 物流 ${row.logisticsCount} 份`:`已提交 ${row.procurementCount} 份 · ${row.body.delivery||'未填写交货时间'}`}}</small></button></template></el-table-column>
    <el-table-column v-if="view==='PROCUREMENT'" class-name="hierarchy-secondary-column" label-class-name="hierarchy-secondary-column" :label="t('inquiryWorkspace.productCount')" width="95" align="center"><template #default="{row}">{{row.body.products?.length||0}}</template></el-table-column>
    <el-table-column class-name="hierarchy-secondary-column" label-class-name="hierarchy-secondary-column" :label="view==='QUOTATIONS'?'报价进度':t('inquiryWorkspace.submittedFactoryQuotes')" min-width="165"><template #default="{row}"><span v-if="view==='QUOTATIONS'" class="quote-progress">采购 {{row.procurementCount}} 份 · 物流 {{row.logisticsCount}} 份</span><span v-else class="quote-progress">已提交 {{row.procurementCount}} 份</span></template></el-table-column>
    <el-table-column v-if="view==='PROCUREMENT'" class-name="hierarchy-secondary-column" label-class-name="hierarchy-secondary-column" prop="body.delivery" :label="t('inquiryWorkspace.delivery')" min-width="120"><template #default="{row}">{{row.body.delivery||'—'}}</template></el-table-column>
    <el-table-column class-name="hierarchy-time-column" label-class-name="hierarchy-time-column" :label="t('inquiryWorkspace.submittedAt')" min-width="155"><template #default="{row}">{{displayTime(row.submittedAt)}}</template></el-table-column>
    <el-table-column :label="t('common.status')" min-width="100"><template #default="{row}"><el-tag effect="light" :type="view==='QUOTATIONS'?inquiryListStatus(row).type:(row.procurementCount>0?'success':'warning')">{{view==='QUOTATIONS'?inquiryListStatus(row).label:stateLabel(row)}}</el-tag></template></el-table-column>
    <el-table-column :label="t('common.actions')" :min-width="view==='PROCUREMENT'?170:120" align="center"><template #default="{row}"><el-button :link="view==='QUOTATIONS'" :plain="view==='PROCUREMENT'" type="primary" @click.stop="open(row)">{{listActionLabel(row)}}</el-button></template></el-table-column>
   </el-table>
   <el-table v-else class="inquiry-list-table" :data="items" stripe @row-dblclick="openFromRow"><el-table-column :label="t('inquiryWorkspace.number')" min-width="190"><template #default="{row}"><span class="inquiry-number">{{displayInquiryNumber(row)}}</span></template></el-table-column><el-table-column prop="body.customer" :label="t('inquiryWorkspace.customer')" min-width="175" show-overflow-tooltip/><el-table-column prop="owner" :label="t('inquiryWorkspace.owner')" min-width="105"/><el-table-column :label="t('inquiryWorkspace.productCount')" min-width="95"><template #default="{row}">{{row.body.products?.length||0}}</template></el-table-column><el-table-column :label="t('inquiryWorkspace.totalQuantity')" min-width="120"><template #default="{row}">{{productTotal(row.body.products,'quantity','unit')}}</template></el-table-column><el-table-column :label="t('inquiryWorkspace.weightVolume')" min-width="180"><template #default="{row}">{{productTotal(row.body.products,'weight')}} / {{productTotal(row.body.products,'volume')}}</template></el-table-column><el-table-column prop="body.delivery" :label="t('inquiryWorkspace.delivery')" min-width="120"/><el-table-column prop="body.loadingPort" :label="t('inquiryWorkspace.loadingPort')" min-width="120"/><el-table-column prop="body.destinationPort" :label="t('inquiryWorkspace.destinationPort')" min-width="120"/><el-table-column prop="body.incoterm" :label="t('inquiryWorkspace.incoterm')" min-width="100"/><el-table-column :label="t('inquiryWorkspace.submittedLogisticsQuotes')" min-width="135"><template #default="{row}">{{row.logisticsCount}}</template></el-table-column><el-table-column :label="t('inquiryWorkspace.submittedAt')" min-width="160"><template #default="{row}">{{displayTime(row.submittedAt)}}</template></el-table-column><el-table-column :label="t('common.status')" min-width="145"><template #default="{row}"><el-tag effect="light" :type="row.state==='WITHDRAWN'?'info':row.state==='INQUIRING'?'success':'warning'">{{stateLabel(row)}}</el-tag></template></el-table-column><el-table-column :label="t('common.actions')" min-width="150" align="center"><template #default="{row}"><el-button plain type="primary" @click.stop="open(row)">{{listActionLabel(row)}}</el-button></template></el-table-column></el-table>
...
async function loadCustomerOptions(){const data=await get<{customers:{id:string;code:string;name:string;shortName:string}[]}>('/sourcing-customer-options',{page_size:500});customerOptions.value=(data.customers||[]).map(option=>({...option,shortName:option.shortName||'',value:option.name}))}
