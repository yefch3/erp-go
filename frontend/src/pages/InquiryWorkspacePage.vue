...
const states=computed(()=>department.value?[{value:'WAITING',label:'待报价'},{value:'QUOTED',label:'已有报价'}]:view.value==='SALES'?[{value:'UNSUBMITTED',label:'未提交'},{value:'INQUIRING',label:'询价中'},{value:'WITHDRAWN',label:'已撤回'}]:[])
...
async function loadList(){const r=await command('list',{page:page.value,size:size.value,keyword:keyword.value,state:state.value});items.value=r.items;total.value=r.total}
...
function initialDetailTab(){return view.value==='SALES'?'overview':view.value==='QUOTATIONS'?'quotes':'products'}
