<template>
 <main class="inquiry-workspace" v-loading="busy">
  ...
  <template v-else>
   ...
   <section v-if="editor" class="quote-editor"><h3>{{view==='PROCUREMENT'?'工厂报价':'货代报价'}}</h3><el-form label-position="top" class="inquiry-fields"><template v-if="view==='PROCUREMENT'"><el-form-item label="工厂或供应商"><el-autocomplete v-model="editor.body.company" :fetch-suggestions="companySuggestions" placeholder="选择已有数据或直接输入" :trigger-on-focus="true"/></el-form-item><el-form-item label="币种"><el-input v-model="editor.body.currency" maxlength="3"/></el-form-item><el-form-item label="统一预计交货日期"><el-date-picker v-model="editor.body.delivery" type="date" value-format="YYYY-MM-DD"/></el-form-item></template><template v-else><el-form-item v-for="f in logisticsPrimaryFields" :key="f.key" :label="f.label"><el-input v-model="editor.body[f.key]"/></el-form-item><el-form-item label="预计开船日期"><el-date-picker v-model="editor.body.departure" value-format="YYYY-MM-DD"/></el-form-item><el-form-item label="预计到港日期"><el-date-picker v-model="editor.body.arrival" value-format="YYYY-MM-DD"/></el-form-item></template></el-form>
