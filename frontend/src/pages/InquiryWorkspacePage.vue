...
function customerShortName(row:Inquiry){const customerId=String(row.body.customerId||'');return customerOptions.value.find(option=>option.id===customerId)?.shortName?.trim()||row.body.customer?.trim()||'—'}
...
async function loadList(){
 ...
 if(view.value==='QUOTATIONS'&&state.value){
  ...
  do{const r=await command('list',{page:sourcePage,size:100,keyword:keyword.value,state:''});if(request!==listRequest)return;sourceTotal=r.total;matches.push(...r.items.filter(i=>state.value==='HAS_QUOTES'?inquiryHasQuote(i):!inquiryHasQuote(i)));if(!r.items.length)break;sourcePage++}while((sourcePage-1)*100<sourceTotal)
...
async function loadCustomerOptions(keyword=''){const data=await get<{customers:{id:string;code:string;name:string;shortName:string}[]}>('/sourcing-customer-options',{page_size:200,keyword});customerOptions.value=(data.customers||[]).map(option=>({...option,shortName:option.shortName||'',value:option.name}))}
async function customerSuggestions(query:string,done:(items:CustomerOption[])=>void){try{await loadCustomerOptions(query);const needle=query.trim().toLowerCase();done(customerOptions.value.filter(option=>!needle||`${option.code} ${option.name}`.toLowerCase().includes(needle)).slice(0,50))}catch{done([])}}
