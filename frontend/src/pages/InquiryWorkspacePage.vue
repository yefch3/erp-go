    ...
    <el-tabs v-model="detailTab" class="detail-tabs">
     ...
     <section v-if="!editor" class="business-card quotes-section">
      ...
      <el-table :data="item.quotes||[]" :max-height="500" stripe :empty-text="t('inquiryWorkspace.quotes.empty')">
       ...
       <el-table-column :label="t('inquiryWorkspace.quotes.type')" width="100"><template #default="{row:q}"><el-tag size="small" :type="q.kind==='PROCUREMENT'?'warning':'success'">{{t(q.kind==='PROCUREMENT'?'inquiryWorkspace.quotes.factory':'inquiryWorkspace.quotes.forwarder')}}</el-tag></template></el-table-column>
...
const quoteSectionTitle=computed(()=>t(view.value==='QUOTATIONS'?'inquiryWorkspace.quotes.comparison':view.value==='PROCUREMENT'?'inquiryWorkspace.quotes.factoryQuote':'inquiryWorkspace.quotes.shippingQuote'))
