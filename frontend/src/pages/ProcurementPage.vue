  ...
  <div class="workbench">
    <header class="hero">
      ...
      <div class="hero-actions">
        <a class="template-link" href="/templates/procurement-standard-inquiry-template.csv" download="采购标准询盘模板.csv"><el-button>{{ t('procurementWorkbench.downloadTemplate') }}</el-button></a>
        <el-button v-if="canSourcingWrite" @click="openManualUpload">{{ t('procurementWorkbench.uploadInquiry') }}</el-button>
    ...
    <section class="intake-band">
      <button class="source" :disabled="!canMailRead" @click="router.push('/emails')"><span>✉</span><div><strong>{{ t('procurementWorkbench.mailTransfer') }}</strong><small>{{ t('procurementWorkbench.mailTransferHint') }}</small><em :class="`mail-state mail-state--${mailState}`">{{ t(`procurementWorkbench.mailStates.${mailState}`) }}</em><em>{{ t('procurementWorkbench.openMail') }} →</em></div></button><b>＋</b>
    ...
    <section>
      <div class="card focus-card">
        ...
        <div v-else class="empty-focus">
          ...
          <div><el-button v-if="canSourcingWrite" @click="openManualUpload">{{ t('procurementWorkbench.uploadInquiry') }}</el-button><el-button v-if="canMailRead" @click="router.push('/emails')">{{ t('procurementWorkbench.openMail') }}</el-button></div>
        </div>
      </div>
    </section>
  </div>
...
const canMailRead = auth.can('mail:email:read')
const metrics = reactive({ pending: 0, reviewing: 0, sourcing: 0, requirements: 0, orders: 0 })
const mailState = ref<'connected' | 'error' | 'unavailable'>('unavailable')
...
async function loadMailState() {
  if (!canMailRead) return
  try {
    const data = await get<{ account?: { email?: string; lastError?: string } }>('/my-mail-account')
    mailState.value = data.account?.email ? (data.account.lastError ? 'error' : 'connected') : 'unavailable'
  } catch { mailState.value = 'unavailable' }
}
