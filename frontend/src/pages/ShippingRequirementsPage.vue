    ...
    <el-dialog v-model="detailOpen" fullscreen :title="`${mode === 'inquiry' ? t('shipping.executionInquiryTitle') : t('shipping.ordersTitle')} · ${detail?.contractNo || ''}`" destroy-on-close class="shipping-fullscreen-detail">
      ...
      <template v-if="detail">
        ...
        <section v-if="mode === 'orders'" class="stage-card" :class="{ 'is-current': ['APPROVED','CONTRACT_UPLOADED'].includes(detail.status) }">
          ...
          <el-alert v-if="detail.status === 'PAYMENT_REQUESTED'" type="success" :closable="false" :title="t('shipping.delegatedHint')" />
        </section>
      </template>
      </div>
    </el-dialog>
...
const statuses=computed(()=>mode.value==='orders'?['DRAFT','PENDING_APPROVAL','APPROVED','CONTRACT_UPLOADED','PAYMENT_REQUESTED']:['WAITING_REQUOTE','RETURNED']), currencies=['USD','CNY','EUR','GBP','CAD','AUD','HKD']
...
function statusLabel(v:string){return t(`shipping.handoffStatuses.${v}`)} function statusType(v:string){if(v==='RETURNED')return 'danger';if(v==='WAITING_REQUOTE'||v==='DRAFT'||v==='PENDING_APPROVAL'||v==='CONTRACT_UPLOADED')return 'warning';if(v==='PAYMENT_REQUESTED')return 'success';return 'info'} function nextLabel(r:Handoff){if(['WAITING_REQUOTE','RETURNED'].includes(r.status))return t('shipping.d4Start');if(r.status==='DRAFT')return t('shipping.editDraft');if(r.status==='APPROVED')return t('shipping.d4Upload');return t('shipping.d4View')}
...
async function load(){loading.value=true;try{const d=await get<{handoffs:Handoff[]}>('/shipping/contract-handoffs');rows.value=(d.handoffs??[]).filter(r=>status.value==='CONTRACT'?['APPROVED','CONTRACT_UPLOADED'].includes(r.status):status.value?r.status===status.value:statuses.value.includes(r.status));page.value=1}finally{loading.value=false}} async function loadParties(){const [f,c]=await Promise.all([get<{suppliers:Party[]}>('/suppliers',{page_size:200,status:'ACTIVE',business_type:'FORWARDER'}),get<{suppliers:Party[]}>('/suppliers',{page_size:200,status:'ACTIVE',business_type:'CARRIER'})]);forwarders.value=f.suppliers??[];carriers.value=c.suppliers??[]}
...
async function openRoutedHandoff(){const id=String(route.query.handoff??'');if(!id)return;let row=rows.value.find(item=>item.id===id);if(!row&&mode.value==='orders'){const d=await get<{handoff:Handoff}>(`/shipping/contract-handoffs/${id}`);const mapped=['APPROVED','CONTRACT_UPLOADED'].includes(d.handoff.status)?'CONTRACT':d.handoff.status;if(['DRAFT','PENDING_APPROVAL','CONTRACT','PAYMENT_REQUESTED'].includes(mapped)){status.value=mapped;await load();row=rows.value.find(item=>item.id===id)}}if(row)await openDetail(row)}
