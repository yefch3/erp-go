...
if($gitPointer -notmatch '^gitdir: (.+)'){throw 'Linked D4 worktree required'}
$gitDir=$Matches[1].Trim()
if((Get-Content -LiteralPath (Join-Path $gitDir 'HEAD') -Raw).Trim() -ne 'ref: refs/heads/codex/erp-v1-d4-20260909'){throw 'D4 branch required'}
$out=Join-Path $root '.d4-environment'
New-Item -ItemType Directory -Path $out -Force | Out-Null
$file=Join-Path $out 'compose.json'
if ($Action -eq 'Prepare') {
    $emptyEnv=Join-Path $out 'empty.env'
    [IO.File]::WriteAllText($emptyEnv,'')
    $env:INTERNAL_SIGNING_KEY='d4-isolated-test-signing-key-20260909-only'
    ...
    foreach($name in @($config.services.Keys)) {
        ...
        $service.ports=@()
        if($service.ContainsKey('build')) { $service.image="erp-d4-20260909/${name}:worktree" }
        if($service.ContainsKey('environment')) {
            ...
            if($e.ContainsKey('JWT_SECRET')){$e.JWT_SECRET='d4-isolated-jwt-20260909-only'}
            if($e.ContainsKey('MINIO_BUCKET')){$e.MINIO_BUCKET='erp-d4-files'}
            if($e.ContainsKey('MINIO_PUBLIC_ENDPOINT')){$e.MINIO_PUBLIC_ENDPOINT='localhost:29201'}
            if($e.ContainsKey('MAIL_PROVIDER')){$e.MAIL_PROVIDER='dev'}
            if($e.ContainsKey('ADMIN_EMAIL')){$e.ADMIN_EMAIL='admin@d4.example.test'}
            if($e.ContainsKey('COMPANY_MAIL_DOMAINS')){$e.COMPANY_MAIL_DOMAINS='d4.example.test'}
            if($e.ContainsKey('COMPANY_NAME')){$e.COMPANY_NAME='D4 isolated acceptance'}
            if($e.ContainsKey('FRONTEND_BASE_URL')){$e.FRONTEND_BASE_URL='http://localhost:25374'}
    ...
    foreach($entry in @{postgres=25634;redis=26581;minio=29201;gateway=28281}.GetEnumerator()) {
    ...
    $config.services.kafka.environment.KAFKA_ADVERTISED_LISTENERS='INTERNAL://kafka:9092,EXTERNAL://localhost:29293'
...
if($c.name -ne 'erp-d4-20260909'){throw 'Unexpected project'}
...
if($Action -eq 'StartInfra'){
    ...
    if($LASTEXITCODE){throw 'D4 infrastructure startup failed'}
}
if($Action -eq 'CheckRuntime'){
    $project='erp-d4-20260909'
    $ids=@(docker ps -aq --filter "label=com.docker.compose.project=$project")
    if($LASTEXITCODE -or $ids.Count -lt 15){throw 'D4 runtime is incomplete'}
    $containers=(docker inspect @ids | ConvertFrom-Json)
    if($LASTEXITCODE){throw 'Cannot inspect D4 containers'}
    $evidence=@()
    foreach($container in $containers){
        if(!$container.State.Running){throw "Stopped D4 container: $($container.Name)"}
        $service=$container.Config.Labels.'com.docker.compose.service'
        $networks=@($container.NetworkSettings.Networks.PSObject.Properties.Name)
        foreach($network in $networks){
            if($network -notin @("${project}_isolated","${project}_convert","${project}_ingress")){throw 'Container joined a non-D4 network'}
        ...
        foreach($mount in $container.Mounts){
            if($mount.Type -eq 'volume'){
                ...
                foreach($owner in $owners){if($owner -notin $ids){throw 'Data volume is attached outside D4'}}
        ...
        $evidence += [ordered]@{service=$service;container=$container.Name;image=$container.Config.Image;running=$container.State.Running;networks=$networks;volumes=@($container.Mounts | Where-Object Type -eq 'volume' | ForEach-Object Name);databaseTarget=if($db.Count){'postgres:5432 (D4 network)'}else{$null}}
    }
    $net=(docker network inspect "${project}_isolated" | ConvertFrom-Json)
    if($LASTEXITCODE -or !$net.Internal){throw 'D4 database network is not internal'}
    $health=Invoke-WebRequest -Uri 'http://127.0.0.1:28281/api/healthz' -TimeoutSec 15
    if($health.StatusCode -ne 200){throw 'D4 gateway health failed'}
    ...
    Write-Output "Runtime verified: $($containers.Count) containers; D4-only networks and volumes; isolated database targets; gateway health 200."
}
