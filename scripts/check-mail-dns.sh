...
# Postfix」的人的。照抄会不会出事，**取决于邮箱设置里的 SMTP 服务器填的是
# 谁**——这是唯一的分岔口：
#
#   * 填的是服务商（smtp.263.net / smtp.exmail.qq.com 这类，也就是
#     MailHostDialog.vue 里那几个预设）：信由服务商的机器投递给收件方，
#     收件方做 SPF 校验时看到的是**服务商的 IP**。此时把这台 VPS 的 IP
#     写进 SPF 不解决任何问题；而如果照教程把原来的 include 换成了 ip4:，
#     全公司的信当场开始 SPF 失败。DKIM 也一样：密钥由服务商生成、在他们
#     后台开启，本仓库没有任何出站签名代码。
#
#   * 填的是自己的机器（smtpHost 是自由文本框，不是白名单，所以这是允许
#     的）：那台机器才是真正的发信方，教程那一套就适用——但适用的对象是
#     **那台机器**，不一定是跑 ERP 的这台。
#
#   * DMARC 是纯策略，两种情况都一样，但它依赖前两条先立起来。
#
# 先弄清楚 SMTP 服务器填的是谁，再决定怎么写记录。
#
# 用法：
