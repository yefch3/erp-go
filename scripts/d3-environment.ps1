...
if($gitPointer -notmatch '^gitdir: (.+)'){throw 'Linked D3 worktree required'}
$gitDir=$Matches[1].Trim()
if((Get-Content -LiteralPath (Join-Path $gitDir 'HEAD') -Raw).Trim() -ne 'ref: refs/heads/codex/erp-v1-d3-20260908'){throw 'D3 branch required'}
$out=Join-Path $root '.d3-environment'
New-Item -ItemType Directory -Path $out -Force | Out-Null
$file=Join-Path $out 'compose.json'
if ($Action -eq 'Prepare') {
    $emptyEnv=Join-Path $out 'empty.env'
    [IO.File]::WriteAllText($emptyEnv,'')
    $env:INTERNAL_SIGNING_KEY='d3-isolated-test-signing-key-20260908-only'
    ...
    foreach($name in @($config.services.Keys)) {
        ...
        $service.ports=@()
        if($service.ContainsKey('build')) { $service.image="erp-d3-20260908/${name}:worktree" }
        if($service.ContainsKey('environment')) {
            ...
            if($e.ContainsKey('JWT_SECRET')){$e.JWT_SECRET='d3-isolated-jwt-20260908-only'}
            if($e.ContainsKey('MINIO_BUCKET')){$e.MINIO_BUCKET='erp-d3-files'}
            if($e.ContainsKey('MINIO_PUBLIC_ENDPOINT')){$e.MINIO_PUBLIC_ENDPOINT='localhost:29101'}
            if($e.ContainsKey('MAIL_PROVIDER')){$e.MAIL_PROVIDER='dev'}
            if($e.ContainsKey('ADMIN_EMAIL')){$e.ADMIN_EMAIL='admin@d3.example.test'}
            if($e.ContainsKey('COMPANY_MAIL_DOMAINS')){$e.COMPANY_MAIL_DOMAINS='d3.example.test'}
            if($e.ContainsKey('COMPANY_NAME')){$e.COMPANY_NAME='D3 isolated acceptance'}
            if($e.ContainsKey('FRONTEND_BASE_URL')){$e.FRONTEND_BASE_URL='http://localhost:25274'}
    ...
    foreach($entry in @{postgres=25534;redis=26481;minio=29101;gateway=28181}.GetEnumerator()) {
    ...
    $config.services.kafka.environment.KAFKA_ADVERTISED_LISTENERS='INTERNAL://kafka:9092,EXTERNAL://localhost:29193'
...
if($c.name -ne 'erp-d3-20260908'){throw 'Unexpected project'}
...
if($Action -eq 'StartInfra'){
    ...
    if($LASTEXITCODE){throw 'D3 infrastructure startup failed'}
}
if($Action -eq 'CheckRuntime'){
    $project='erp-d3-20260908'
    $ids=@(docker ps -aq --filter "label=com.docker.compose.project=$project")
    if($LASTEXITCODE -or $ids.Count -lt 15){throw 'D3 runtime is incomplete'}
    $containers=(docker inspect @ids | ConvertFrom-Json)
    if($LASTEXITCODE){throw 'Cannot inspect D3 containers'}
    $evidence=@()
    foreach($container in $containers){
        if(!$container.State.Running){throw "Stopped D3 container: $($container.Name)"}
        $service=$container.Config.Labels.'com.docker.compose.service'
        $networks=@($container.NetworkSettings.Networks.PSObject.Properties.Name)
        foreach($network in $networks){
            if($network -notin @("${project}_isolated","${project}_convert","${project}_ingress")){throw 'Container joined a non-D3 network'}
        ...
        foreach($mount in $container.Mounts){
            if($mount.Type -eq 'volume'){
                ...
                foreach($owner in $owners){if($owner -notin $ids){throw 'Data volume is attached outside D3'}}
        ...
        $evidence += [ordered]@{service=$service;container=$container.Name;image=$container.Config.Image;running=$container.State.Running;networks=$networks;volumes=@($container.Mounts | Where-Object Type -eq 'volume' | ForEach-Object Name);databaseTarget=if($db.Count){'postgres:5432 (D3 network)'}else{$null}}
    }
    $net=(docker network inspect "${project}_isolated" | ConvertFrom-Json)
    if($LASTEXITCODE -or !$net.Internal){throw 'D3 database network is not internal'}
    $health=Invoke-WebRequest -Uri 'http://127.0.0.1:28181/api/healthz' -TimeoutSec 15
    if($health.StatusCode -ne 200){throw 'D3 gateway health failed'}
    ...
    Write-Output "Runtime verified: $($containers.Count) containers; D3-only networks and volumes; isolated database targets; gateway health 200."
}
