...
func (h *Handler) ListRequirements(ctx context.Context, req *prv1.ListRequirementsRequest) (*prv1.ListRequirementsResponse, error) {
	...
	for _, r := range rows {
		out = append(out, &prv1.Requirement{
			...
			RequiredQty: r.RequiredQty, OrderedQty: r.OrderedQty,
			ReservedQty: r.ReservedQty, AvailableQty: r.AvailableQty, ReceivedQty: r.ReceivedQty,
			...
			SourceValidUntil: r.SourceValidUntil, SourceSalesRemark: r.SourceSalesRemark,
		})
	}
