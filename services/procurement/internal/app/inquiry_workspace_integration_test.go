...
func TestD1InquiryLifecycle(t *testing.T) {
	...
	logistics := InquiryQuoteBody{Company: "Forwarder", Incoterm: "CFR", ExchangeRates: map[string]string{"CNY": "7"}, Charges: []InquiryCharge{{Name: "Ocean", Amount: "10.05", Quantity: "3", Currency: "USD"}, {Name: "Port", Amount: "21.11", Quantity: "2", Currency: "CNY"}}}
	call(301, InquiryCommand{Action: "quote", View: "LOGISTICS", ID: current.ID, Revision: rev, Quote: logistics, Submit: true})
	sales = call(101, InquiryCommand{Action: "get", View: "QUOTATIONS", ID: current.ID})
	if len(sales.Quotes) != 2 || sales.Quotes[1].Body.Totals["USD"] != "30.15" || sales.Quotes[1].Body.Totals["CNY"] != "42.22" || sales.Quotes[1].Body.TotalUSD != "36.18" {
