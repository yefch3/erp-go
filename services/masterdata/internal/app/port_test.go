...
func TestValidatePortInput(t *testing.T) {
	tests := []struct {
		...
		{name: "仅填写英文名称", in: PortInput{Port: Port{UNLOCODE: "CNSHA", CountryCode: "CN", NameEN: "Shanghai", Timezone: "Asia/Shanghai"}}},
		{name: "名称全部缺失", in: PortInput{Port: Port{UNLOCODE: "CNSHA", CountryCode: "CN", Timezone: "Asia/Shanghai"}}, bad: true},
		{name: "港口类型无效", in: PortInput{Port: Port{UNLOCODE: "CNSHA", CountryCode: "CN", NameZH: "上海港", Timezone: "Asia/Shanghai", PortType: "SPACEPORT"}}, bad: true},
		{name: "经纬度越界", in: PortInput{Port: Port{UNLOCODE: "CNSHA", CountryCode: "CN", NameZH: "上海港", Timezone: "Asia/Shanghai", HasCoordinates: true, Latitude: 91}}, bad: true},
	}
...
func TestPortLifecycle(t *testing.T) {
	...
	defer pool.Close()
	svc := New(pool)
	created, err := svc.CreatePort(ctx, 1, PortInput{Port: Port{UNLOCODE: "ZZTST", CountryCode: "ZZ", NameZH: "测试港", NameEN: "Test Port", City: "Test City", Timezone: "UTC", Aliases: []string{"Old Test Port"}, PortType: "SEAPORT", AdminArea: "Test State", HasCoordinates: true, Latitude: 12.3, Longitude: 45.6, IsFavorite: true}, OperatorID: 1, OperatorName: "test"})
	if err != nil {
	...
	changedCode := updated
	changedCode.UNLOCODE = "ZZTS2"
	if _, err = svc.UpdatePort(ctx, 1, PortInput{Port: changedCode, OperatorID: 1, OperatorName: "test"}); err == nil {
		t.Fatal("changing UN/LOCODE should be rejected")
	}
...
func TestPortImport(t *testing.T) {
	...
	svc := New(pool)
	rows := []PortImportRow{
		{RowNumber: 2, ProfileFieldsPresent: true, PortInput: PortInput{Port: Port{UNLOCODE: "ZZI01", CountryCode: "ZZ", NameZH: "导入港一", NameEN: "Import One", Timezone: "UTC", PortType: "RIVER_PORT", AdminArea: "测试州", Latitude: 12.34, Longitude: 56.78, HasCoordinates: true, IsFavorite: true}}},
	...
	var portType, adminArea string
	var latitude, longitude float64
	var hasCoordinates, isFavorite bool
	if err = pool.QueryRow(ctx, "SELECT port_type,admin_area,latitude,longitude,has_coordinates,is_favorite FROM ports WHERE tenant_id=1 AND unlocode='ZZI01'").Scan(&portType, &adminArea, &latitude, &longitude, &hasCoordinates, &isFavorite); err != nil || portType != "RIVER_PORT" || adminArea != "测试州" || latitude != 12.34 || longitude != 56.78 || !hasCoordinates || !isFavorite {
		t.Fatalf("imported profile type=%q area=%q lat=%v lng=%v coordinates=%v favorite=%v err=%v", portType, adminArea, latitude, longitude, hasCoordinates, isFavorite, err)
	}
