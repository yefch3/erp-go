...
WHERE (r.code='LOGISTICS' AND p.code IN ('shipping:sourcing:read','shipping:sourcing:write'))
   OR (r.code='SHIPPING_MANAGER' AND p.code IN ('shipping:sourcing:read','shipping:sourcing:write','shipping:sourcing:approve','shipping:schedule:read','shipping:schedule:write'))
...
-- +goose Down
DELETE FROM role_data_scopes WHERE module='shipping' AND role_id IN (SELECT id FROM roles WHERE code='SHIPPING_MANAGER');
