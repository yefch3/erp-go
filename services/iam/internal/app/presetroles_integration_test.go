...
//
// 一次性库还有第二种红法（2026-09-16）：它跨 worktree 共用、从不重建，所以它走的
// 是**升级路径**而不是全新安装——和生产一样。main 在 9-11 把 59 号用作登录名
// 登录，9-13 的重建把 59 号换成 BOSS 询盘授权、登录名改号 71 并加 72 号和解；
// 已经应用过旧 59 的库会跳过新 59，于是 61/64 给 BOSS 发权限时角色还不存在、
// 静默插 0 行，最后 BOSS 只剩一条权限。全新库（GitHub CI）看不到，这里能看到，
// 而且是稳定红、不是随机红——那是升级路径上的真 bug，修法是补一条和解迁移
// （00073），不是重置影子库。另外单独跑之前先确认没被 skip：没导出
// IAM_MIGRATION_TEST_DSN 时这条测试是 SKIP，看着也是绿的。
