...
-- name: ClearMailAccountFailure :exec
-- 信箱又能用了：清掉记下的错误。
--
...
-- name: SetInboundAnsweredByUID :exec
...
-- name: MarkInboundAnsweredByMessageID :many
...
WHERE tenant_id = sqlc.arg(tenant_id)::bigint
  AND owner_id = sqlc.arg(owner_id)::bigint
  AND account_id = sqlc.arg(account_id)::bigint
  AND message_id = trim(both '<>' from sqlc.arg(in_reply_to)::text)
  AND message_id <> ''
  AND NOT is_answered
...
FROM (
    SELECT m.id, m.from_email, m.from_name, m.subject, m.snippet, m.thread_key,
           ...
           (CASE WHEN sqlc.arg(star_first)::boolean
                   THEN CASE WHEN t.any_starred = (sqlc.arg(sort_dir)::text = 'desc')
                             THEN '1' ELSE '0' END
                   ELSE '' END
            || CASE WHEN sqlc.arg(unread_first)::boolean
                    THEN CASE WHEN t.any_unread = (sqlc.arg(sort_dir)::text = 'desc')
                              THEN '1' ELSE '0' END
                    ELSE '' END
            || CASE sqlc.arg(sort_by)::text
...
WHERE tenant_id = sqlc.arg(tenant_id)::bigint
  ...
  AND CASE WHEN auth_failed THEN
        ...
        OR status_checked_at <= now() - make_interval(secs => sqlc.arg(auth_retry_seconds)::int)
      ELSE
