...
func (s *Service) UpdateProgress(ctx context.Context, tenantID, id int64, in ProgressInput, op Operator) (store.ShippingSchedule, []store.ShippingRouteNode, []store.ShippingDelayEvent, error) {
	...
	err := pgdb.InTx(ctx, s.pool, func(ctx context.Context, tx pgx.Tx) error {
		...
		case "UPDATE_ETA":
			...
			if impact == "PORT" && in.AffectedNodeID == 0 { return apierr.Invalid("SHIPPING_DELAY_PORT_REQUIRED", "港口延误必须选择受影响港口") }
			if impact == "LEG" && (in.FromNodeID == 0 || in.ToNodeID == 0) { return apierr.Invalid("SHIPPING_DELAY_LEG_REQUIRED", "航段延误必须选择起点和终点") }
