...
const handoffSelect = `SELECT id,contract_id,contract_no,contract_version_id,version_no,customer_id,customer_name,batch_no,shipment_group_key,carrier_forwarder,service_option_name,customer_managed,currency,freight_amount::text,charge_basis,port_of_loading,port_of_discharge,coalesce(estimated_departure::text,''),coalesce(estimated_arrival::text,''),coalesce(valid_until::text,''),remark,source_sales_remark,status,coalesce(schedule_id,0),created_at::text,final_forwarder_id,final_forwarder_name,actual_carrier_id,actual_carrier_name,final_service_option,final_currency,final_freight_amount::text,coalesce(final_etd::text,''),coalesce(final_eta::text,''),payment_terms,forwarder_contract_no,coalesce(approval_instance_id,0),return_reason,operator_name,signed_contract_key,signed_contract_name,coalesce(signed_contract_uploaded_at::text,''),coalesce(contract_verified_at::text,''),contract_verified_by_name,coalesce(payment_requested_at::text,''),updated_at::text,coalesce((SELECT m.order_no FROM manual_shipping_orders m WHERE m.tenant_id=contract_shipping_handoffs.tenant_id AND m.handoff_id=contract_shipping_handoffs.id),''),coalesce((SELECT m.amount_missing FROM manual_shipping_orders m WHERE m.tenant_id=contract_shipping_handoffs.tenant_id AND m.handoff_id=contract_shipping_handoffs.id),false),coalesce((SELECT m.linked_contract_id FROM manual_shipping_orders m WHERE m.tenant_id=contract_shipping_handoffs.tenant_id AND m.handoff_id=contract_shipping_handoffs.id),0) FROM contract_shipping_handoffs`

func scanHandoff(row pgx.Row) (ContractHandoff, error) {
	var v ContractHandoff
	err := row.Scan(&v.ID, &v.ContractID, &v.ContractNo, &v.ContractVersionID, &v.VersionNo, &v.CustomerID, &v.CustomerName, &v.BatchNo, &v.ShipmentGroupKey, &v.CarrierForwarder, &v.ServiceOptionName, &v.CustomerManaged, &v.Currency, &v.FreightAmount, &v.ChargeBasis, &v.PortOfLoading, &v.PortOfDischarge, &v.EstimatedDeparture, &v.EstimatedArrival, &v.ValidUntil, &v.Remark, &v.SourceSalesRemark, &v.Status, &v.ScheduleID, &v.CreatedAt, &v.FinalForwarderID, &v.FinalForwarderName, &v.ActualCarrierID, &v.ActualCarrierName, &v.FinalServiceOption, &v.FinalCurrency, &v.FinalFreightAmount, &v.FinalETD, &v.FinalETA, &v.PaymentTerms, &v.ForwarderContractNo, &v.ApprovalInstanceID, &v.ReturnReason, &v.OperatorName, &v.SignedContractKey, &v.SignedContractName, &v.SignedContractUploadedAt, &v.ContractVerifiedAt, &v.ContractVerifiedByName, &v.PaymentRequestedAt, &v.UpdatedAt, &v.ManualOrderNo, &v.AmountMissing, &v.LinkedContractID)
	return v, err
}
