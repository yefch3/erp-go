...
& (Join-Path $root 'scripts/d3-environment.ps1') -Action CheckRuntime
$base='http://127.0.0.1:28181'
$password='D3-Acceptance-2026!'
...
function WaitRelease($session,$id){
  ...
  throw "Contract $id did not reach both D3 queues"
}

$admin=Login 'admin@d3.example.test' 'admin123'
$roles=(Api $admin GET '/roles' $null).roles
$people=@{}
foreach($pair in @(@('S1','SALES'),@('B1','BOSS'),@('F1','FINANCE'),@('P1','BUYER'),@('L1','LOGISTICS'))){
  $name=$pair[0];$email="$($name.ToLower())@d3.example.test"
  $employee=(Api $admin GET "/employees?keyword=$name&size=100" $null).employees|Where-Object {$_.code -eq "D3-$name"}|Select-Object -First 1
  if(!$employee){$employee=(Api $admin POST '/employees' @{code="D3-$name";name=$name;email=$email;departmentId='1';initialPassword=$password;username=$email}).employee}
  ...
  docker exec erp-d3-20260908-postgres-1 psql -U erp_iam -d erp_iam -c "UPDATE employees SET email_verified_at=now() WHERE tenant_id=1 AND id=$($employee.id) AND email LIKE '%@d3.example.test'"|Out-Null
...
# scoped to the disposable D3 tenant rather than a product migration default.
docker exec erp-d3-20260908-postgres-1 psql -U erp_iam -d erp_iam -c "UPDATE role_data_scopes s SET scope_type='ALL' FROM roles r WHERE s.tenant_id=1 AND s.role_id=r.id AND r.code='BUYER' AND s.module='procurement_requirement'"|Out-Null

$customer=(Api $admin GET '/customers?keyword=D3%20验收客户&status=ALL&page_size=100' $null).customers|Where-Object {$_.code -eq 'D3-CUSTOMER'}|Select-Object -First 1
if(!$customer){
  $customer=(Api $admin POST '/customers' @{code='D3-CUSTOMER';name='D3 验收客户';country='China';countryCode='CN';currency='USD';paymentTerm='TT';address='D3 isolated data';contacts=@(@{name='验收联系人';email='customer@d3.example.test';phone='+86 13800000000';isPrimary=$true})}).customer
}
...
if(!$contact){throw 'D3 acceptance customer has no contact'}

$accountEvidence=[ordered]@{baseUrl='http://127.0.0.1:25274';gateway='http://127.0.0.1:28181';password=$password;accounts=@{sales=$people.S1.email;boss=$people.B1.email;finance=$people.F1.email;buyer=$people.P1.email;logistics=$people.L1.email}}
...
if($SeedOnly){Write-Output 'D3 accounts and customer ready';return}

function NewExecutingContract([string]$title){
  $s=$people.S1.session;$b=$people.B1.session
  $body=@{title=$title;customerId=[string]$customer.id;customer=$customer.name;contactId=[string]$contact.id;contact=$contact.name;delivery='2026-10-31';loadingPort='Shanghai';destinationPort='Hamburg';incoterm='FOB';remark='D3 isolated acceptance';products=@(
  ...
  Api $b POST "/approvals/tasks/$($task.task.id)/act" @{action='APPROVE';comment='D3 acceptance approved'}|Out-Null
  $contract=WaitContract $s $id 'PENDING_SIGN'
  $upload=Api $s POST "/contracts/$id/files/presign" @{fileName='D3-signed-contract.pdf';contentType='application/pdf'}
  ...
  Api $s POST "/contracts/$id/files" @{fileKey=$upload.fileKey;fileName='D3-signed-contract.pdf';kind='SIGNED';contractVersionId=$contract.version.id;contentType='application/pdf'}|Out-Null
  Api $s POST "/contracts/$id/sign" @{}|Out-Null
  WaitContract $s $id 'EXECUTING'|Out-Null
  return [string]$id
}

$automatedID=NewExecutingContract 'D3 自动回归合同'
...
$decision=Api $people.F1.session POST "/receivable-due/$automatedID/execution-condition" @{conditionType='NO_PREPAYMENT_REQUIRED';note='D3 自动回归'}
...
$manualID=NewExecutingContract 'D3 人工验收合同'
...
$result|ConvertTo-Json -Depth 5|Set-Content (Join-Path $evidenceDir 'd3-api-result.json') -Encoding utf8
Write-Output "D3 automated contract=$automatedID; manual acceptance contract=$manualID"
