...
foreach($pair in @(@('S1','SALES'),@('S2','SALES'),@('P1','BUYER'),@('P2','BUYER'),@('L1','LOGISTICS'),@('L2','LOGISTICS'),@('M1','SALES_MANAGER'),@('B1','BOSS'),@('U1',''))){
 $name=$pair[0];$email="$($name.ToLower())@d2.example.test"
 $existing=(Api $admin GET "/employees?keyword=$name&size=100" $null).employees|Where-Object {$_.code -eq "D2-$name"}|Select-Object -First 1
 if(!$existing){$existing=(Api $admin POST '/employees' @{code="D2-$name";name=$name;email=$email;departmentId='1';initialPassword='D2-Acceptance-2026!';username=$email}).employee}
 ...
 docker exec erp-d2-20260907-postgres-1 psql -U erp_iam -d erp_iam -c "UPDATE employees SET email_verified_at=now() WHERE tenant_id=1 AND id=$($existing.id) AND email LIKE '%@d2.example.test'" | Out-Null
 $people[$name]=@{id=$existing.id;session=(Login $email 'D2-Acceptance-2026!')}
}
