...
**网关没有任何请求速率限制。**
`grep -rniE "rate.?limit|throttl" services/gateway/ services/iam/ pkg/` 返回空。
任何持有有效会话的人，可以用任意速率敲任意接口。

具体风险，按严重程度：

**① 登录接口没有失败限制**（`POST /api/auth/login`，未认证路由）
可以无限次尝试用户名密码。**这是最直接的一条**：一个公网部署的 ERP，登录接口
不限速等于把爆破成本降到零。

**② 邮箱门禁没有失败限制**（`POST /api/mailbox/verify`）
这条更微妙——它拿着密码去**真实邮件服务商**登录。反复失败不只是我们的问题，
是**替攻击者拿我们的 IP 去爆破 Gmail/263**，结果是我们的服务器 IP 被邮局封禁，
影响全公司收发。
