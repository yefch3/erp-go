...
#### 三、Message-ID 出门就被改写，两份记录对不上号

一封发出的信被记录两份：ERP 自己的 `email_messages`（带每位收件人的投递
状态和打开追踪）+ 邮局「已发送」文件夹同步下来的 `email_inbound`。
按 Message-ID 关联是最自然的想法，**行不通**：

```
ERP 写的 Message-ID：  <uuid@domain>
Gmail 发出后变成：      CAANWAZy…@mail.gmail.com
```

线上验证 18 封 ERP 发出、0 条匹配。这不是 bug，是 Gmail 的行为。

后果：**没有单一的「已发送」数据源**。只看 ERP 的，丢掉从别的客户端发的信；
只看邮局的，丢掉刚发出还没同步回来的信（甚至永远丢掉——不是每个邮局都
把 SMTP 发出的信存进 Sent）。所以 `ListSentUnified` 两边合并、按时间排序，
去重条件是**收件人 + 主题 + 十分钟内**，且只到这个程度为止：
差一点没匹配上就显示两条（列表变乱），匹配得太贪心会把人真发出去的信藏起来
（比乱更糟）。行上带 `kind`，因为两种记录点开的是不同的页面。
