...
### 5.4.1 录入已有合同与执行接管（2026-09-02）

业务确认后的现实:**合同是邮件谈的,签好的 PDF 传回来**,报价单这一步根本不存在。
所以合同不能只有「由报价生成」一条入口。

正式入口是 `POST /api/contracts/existing`，与「由报价生成」并存。旧的
`/api/contracts/direct` 暂时只保留接口兼容，不再由页面使用。已有合同的
`quotation_id` 为空，`entry_source = EXISTING_CONTRACT`，系统合同号照常自动生成；
纸质或外部合同号单独存在 `external_contract_no`，允许为空。

这不是另一条“起草合同”流程。录入对象已经在线下签署并生效，保存后直接为
`EXECUTING`，首版直接冻结为 `APPROVED`，不重复草稿、审批和签署。负责销售默认
为录入人；普通销售只能选本人，管理范围内的经理或管理员可以代录并指定负责人。
...
每行除产品、规格、合同数量和对客单价外，还录入已到货和已发运数量；销售不需要知道或录入原采购单价，重建采购订单时未知采购成本先记为 0，后续由采购侧补充；
合同层录入已收款金额，并选择原负责采购和原供应商。已有合同被视为在线下已经
下过采购单，因此采购服务直接重建一张归原采购人员的历史采购单，不经过采购需求、
草稿或审批。历史到货形成到货记录，但不重复向库存发布入库事件。

已有纸质合同中的产品不一定已经建立产品主数据。录入页允许选择现有产品，也允许
直接填写合同上的产品名称和计量单位；后一种保存 `product_id = 0` 的合同快照，
不自动新建产品主数据。采购需求、合同详情和发货统计使用快照名称与单位继续流转。
装运港和目的港同样采用“选择或输入”的轻量方式：有港口读取权限时可搜索活动港口，
也始终允许保留纸质合同原文；自由输入只写入合同版本快照，不新增港口主数据。

采购单数量等于合同数量。已到货为 0 时状态为 `ORDERED`；部分到货时为
`PARTIALLY_RECEIVED`；全部到货时为 `RECEIVED`。待发运和待收款仍分别按
“合同数量－期初已发运”和“合同金额－期初已收款”计算。

合同 PDF 可以当场上传；选择“稍后补传”时合同仍可开始执行，但详情持续显示缺件提醒。
**汇率**在录入时取一次并冻结，和报价单合同相同。

详细业务规则和验收数据见
[《出口合同手工录入与执行接管需求》](./出口合同手工录入与执行接管需求.md)。

**验收基准（2026-09-02）**

```
合同 100 TON，期初已采购 60、已到货 40、已发运 20
保存             → EXECUTING，不出现审批和签署步骤
采购事件         → required_qty = 40 TON
合同进度         → 期初发运 20 TON，剩余发运 80 TON
收款进度         → 合同金额 - 期初已收款
```
...
- **已有合同执行接管已实现**：页面走 `POST /api/contracts/existing`，保存后直接执行；
  期初采购、到货、发运和收款只作为快照，后续只生成未完成余额。
...
系统也支持录入已有合同；这类合同的 `quotation_id` 为空，没有需要连带转移的
报价单。所以下面的链路不变量只约束**由报价生成**的合同，不约束已有合同。
...
| 入口 | 由报价生成的合同可从报价单号或合同号任一端发起并整链转移；已有合同从合同端发起 |
| 留痕 | `ownership_transfers`,一份单据一行；报价链交接产生两行，已有合同交接产生一行 |
...
人工审核：左原文 右解析，逐行确认 / 换产品 / 自己搜 / 标记"库里没有"
  → CONFIRMED → 建立采购询价案件
  → 多工厂询价 / 供应商报价 / 港杂费和海运费 / 成本方案
  → 生成客户报价草稿
```

这里的 `CONFIRMED` 只表示“客户询盘已经被人核对成内部产品与数量”，不表示已经得到
可对客户承诺的价格。售前询价不能进入合同生效后的 `purchase_requirements`；两者之间新增
独立的采购询价与成本核算子域。完整表结构、服务边界和分期见
原《询盘到采购与报价实现方案》（P0–P4 于 2026-08-16 全部落地，文档 2026-09-01 退役；要看当时的设计取舍，翻 git 历史）。
...
## 限速（2026-08-05 盘点）：现在有什么，还缺什么

> 本节是**盘点 + 待决**，不是已完成的设计。已落地的部分标 ✅ 并给出位置，
> 缺口标 ⬜ 并说清楚风险。怎么补需要先讨论。

系统里已经有五种「限制」，但它们**限的不是同一件事**，混在一起谈会得出错的
结论。先分清楚：

| 类型 | 限的是 | 保护谁 |
|---|---|---|
| **配额** | 一段时间内总量 | 对方（邮局的每小时/每日上限） |
| **并发** | 同时进行几个 | 对方 + 我们自己的资源 |
| **退避** | 失败后隔多久再试 | 双方，以及不让坏账占住资源 |
| **超时** | 单次操作等多久 | 我们自己（防止一个卡住拖垮全体） |
| **速率** | 每秒/每分钟几次请求 | **我们自己**（防滥用、防误操作） |

前四种已经有了。**第五种一个都没有。**

### 已落地 ✅

**发送配额**（`services/mail/internal/app/quota.go`）
按邮件服务商设置的每小时/每日上限排队等待，而不是硬撞。达到上限时 worker
让这封信排队，不是失败——邮局的限额是它的规则，撞上去只会换来更长的封禁。

**发送节流**（`MAIL_SEND_DELAY`，默认 100ms）
连续发送之间的间隔。一次群发 500 封不会在两秒内全部砸向邮局。

**合并发送人数上限**（50）
一次 SMTP 事务的收件人数量。超过这个数多数邮局会拒收整封。

**同步并发上限**（`MAIL_SYNC_CONCURRENCY`，默认 8，见 §5.12）
同时同步几个邮箱。上限不是为我们的资源——goroutine 和 socket 都很便宜——而是
「一个客户端能合理地同时向邮件服务商要多少」，以及**炸开范围**：一个卡住的邮箱
占住八分之一而不是全部。

**同步退避 + 失败配额**（`syncbackoff.go`）
失败的邮箱按阶梯延后重试（0 / 2min / 10min / 30min / 2h），且**有失败记录的
邮箱最多占用 fleet 的四分之一**——健康邮箱永远有 3/4 的位置。前者答「多久试
一次」，后者答「最坏能占几个」。

**同步去重**（`syncfleet.go`）
同一邮箱不会同时跑两次同步。这是正确性而非节流：两次同步扫同一批 UID 会让
同步水位线跟自己赛跑。

**超时分级**（`MAIL_DIAL_TIMEOUT` 10s / `MAIL_SYNC_TIMEOUT` 90s）
建连接和等命令响应是两种等待。合用一个数意味着一个没人应门的主机占住 worker
一个命令的耐心。

**其他硬上限**
单附件 10 MB、单封总附件 20 MB、搜索关键词最少 2 字符、
`MAIL_SYNC_HISTORY` 每文件夹 2000 封。

### 失败次数限制 ✅（2026-08-06 补上①②）

`services/gateway/internal/httpapi/throttle.go`。只覆盖两条路由——**猜测次数
无上限本身就是攻击**的那两条。

**① 登录**（`POST /api/auth/login`）：15 分钟内 10 次失败 → 429 + `Retry-After`。
**② 邮箱门禁**（`POST /api/mailbox/verify`）：15 分钟内 **5** 次失败。

②更严，而且是先写它的原因：它拿调用方给的地址和授权码去**真实邮件服务商**
登录。无限猜测不只是我们的数据风险，是**花掉我们服务器 IP 在邮局那里的信用**，
结局是全公司发信地址被封。测试里有一条断言就是钉这个大小关系的。

三个决定，都有测试：

- **按账号计，不按 IP。** 公网部署时全公司走一个出口 IP，按 IP 限会让一个人
  的失败锁死坐他旁边的所有人。被猜的是账号，就该按账号计。
  登录用请求体里的用户名（**不存在的账号也照计**——否则"被锁"本身就成了
  探测账号是否存在的手段）；邮箱门禁用已认证的 `t{tenant}.e{employee}`。
- **失败是连续计数。** 成功即清零：连打九次错然后登进去的人，不是离锁定
  只差一次的人。
- **存储故障时放行。** 与 `UnlockStore` 相反且刻意：门禁缺失意味着
  "无法证明此人控制该邮箱"，必须拒；限速器缺失意味着"无法计数"，
  拒了就是把一次 Redis 抖动变成全公司登不进 ERP。

实现细节两条：键是 `sha256(小写去空格的标识)` 的前 16 字节十六进制——Redis
里不留用户名明文，也不让请求体里的任意字符串进键空间；`EXPIRE` 带 `NX`，
所以窗口内继续猜**不会把窗口往后推**（否则攻击者一直猜，真正的主人就一直被锁）。

**实测**（部署后打真实网关）：同一账号第 10 次失败返回 429 + `Retry-After: 900`，
第 11 次仍 429；同一时刻另一个账号第一次仍是 401 不受影响；再猜一次 TTL
从 878 掉到 873 而不是回到 900；Redis 里的键是哈希，无明文。

### 缺口（2026-08-10 补完 ③④）

**③ ~~「立即收信」没有速率限制~~** ✅ 令牌桶，每人一个（`ratelimit.go`）。
桶容量 3、每 20 秒回一个:连按两下是正常操作，按一下午不是。用桶而不是
固定窗口，是因为窗口会拒掉一对按键里的第二下，然后在翻页那一刻又放出
一整份额度。刷新与扣减在一段 Lua 里完成——分成读改写两步的话，同时到达
的两下会看到同一个令牌并各拿一次，那正是限制要挡的突发。

**④ ~~两条公开路由完全无限制~~** ✅ 按来源计数，每分钟 600（同上文件）。
这里用最便宜的固定窗口:高频、每次极廉价，边界处最坏放过两倍，对带宽
保护无所谓。

**这两个阈值不要随手调紧，有个坑：** Gmail 不让收件人的浏览器直接来取
图，是它自己的服务器代取并缓存。所以一千个 Gmail 收件人到我们这里表现为
少数几个 googleusercontent 地址。按地址限得紧，掐掉的是**所有 Gmail 收件
人的图片和打开统计**，不是攻击者。这两个数字是用来挡扫描器的，不是用来
塑造正常流量的。触发时会打日志，头一次在生产上响的时候，先看清楚抓到的
是扫描器还是某家邮件服务商的代理——两者要相反的处理。

（cid: 内嵌落地之后，新邮件根本不走图片路由，只有历史邮件还走，暴露面比
这两个数字看起来小得多。）

**原文记录如下。**

**③ 「立即收信」没有速率限制**（`SyncNow`）
fleet 的去重挡住了「同时点五次跑五个同步」，但挡不住「每秒点一次」——每次都
是一轮真实的 IMAP 往返。一个卡住的自动刷新脚本就能把自己的邮箱同步打满。

**④ 两条公开路由完全无限制**
- `/api/public/mail-open/{key}` —— 追踪像素，设计上就是给陌生人访问的
- `/api/public/mail-images/{token}` —— 邮件内嵌图片

它们不需要登录（收件人的邮件客户端没有会话），所以**任何人都可以用任意速率
打这两个地址**。当前实现对真假 key 的响应是一致的（不泄露哪些发送存在），
但没有任何速率上限。

**⑤ 搜索没有速率限制**
两字符的中文查询会退化成一次全表扫描（约 90ms，见 §5.12.16）。一个循环调用
就能把数据库连接池占满。

### 待讨论的问题

补限速之前需要先定几件事，它们的答案会决定实现方式：

1. **限速的粒度是什么？** 按用户？按 IP？按会话？
   公网部署时同一家公司可能全部走一个出口 IP，按 IP 限速会误伤全公司。

2. **状态放哪？** 单进程内存（简单，但多副本时每个副本各限各的，等于放宽 N 倍）
   还是 Redis（准确，但多一个依赖在请求路径上）。系统里已经有 Redis
   （`livefeed` 在用），所以这个选择比通常更便宜。

3. **超限时的行为？** 429 拒绝、排队等待、还是逐步加延迟？
   对登录该拒绝，对「立即收信」排队更合理——人只是想刷新，不是在攻击。

4. **公开路由怎么办？** 追踪像素本来就该被陌生人访问，不能按会话限。
   按 IP？按 key？还是干脆接受它，靠上游（nginx/CDN）来限？

5. **哪些接口值得单独设限？** 全局一个数字很难同时适合登录（应该很严）和
   列表查询（应该很松）。

我的倾向：**先做 ① 和 ②**（登录和邮箱门禁的失败限制），因为它们是安全问题
而不是性能问题，而且不需要回答上面全部五个问题——失败计数按账号即可，账号是
明确的主体，不受出口 IP 共享的影响。其余的等有真实流量再定。

---

## 零信任（2026-08-08 盘点）：外层是，内层刻意不是

> 和上一节同样是**盘点 + 待决**。起因是一个直接的问题：「这套系统有零信任吗？」
> 答案是一半有，而没有的那一半是当初的选择，不是疏忽——但上云会改变它的代价。

「零信任」这个词被用滥了，先说它到底要求什么，否则对着营销词自评没有意义。它的
核心只有一句：**不因为你在网络里面就信任你**。展开成三条可检验的要求——每个请求
单独验证、按最小权限逐次授权、假定已经被攻破因而处处留痕。
...
| 模型 | 零信任 | 城墙式：硬壳、软心 |
| 每次请求验身份 | ✅ | ❌ 照单全收 |
| 传输加密 | ✅（上云后 https） | ❌ 全部 `insecure.NewCredentials()` |
| 攻破一处的后果 | 一个账号 | **所有租户的管理员** |
| 挡着的是什么 | 令牌 + 权限 + 数据范围 | **只有网络绑定** |

### 外层：确实是零信任的 ✅

**每个请求单独查权限。** 215 条路由，204 条带 `s.perm(...)`。其余 11 条里 6 条
是真公开的（登录、激活、追踪像素、图片 token、OAuth 回调），5 条是「关于你自己」
的（查自己的权限、改自己的密码、事件流、编号、选项）。授权不是登录时算一次
存起来，是每次请求现算。

**登录了不等于能看邮件**（§邮箱登录面）。这是全系统最典型的一处，也是多数同类
系统不做的：ERP 的登录态只证明「今天早上有人输对了密码」，不证明「此刻坐在键盘
前的是本人」。所以读邮件要用邮箱密码**向邮件服务器真的登录一次**，凭证 12 小时
过期、存 Redis、退出即失效。并且**系统里没有任何接口能让管理员替别人设置邮箱
凭证**——唯一入口是本人现场登录（`/api/mailbox/verify`）。

**数据范围每次现算，且失败时收紧不是放宽。** `visibleTo` 拿不到 iam 的答复就
拒绝——「一个看得见的错误好过悄悄泄露别人的往来」。

**归属写进 SQL，不靠 handler 记得检查。** 会话 key 是可猜的，所以「这封信是不是
你的」由查询的 `owner_id = $caller` 保证。

**问不存在的东西答「不存在」**，不答「无权限」——后者本身就是情报。

**不信任何自称**：转发头默认不信（`TRUST_PROXY_HEADERS`）；图片类型从字节嗅探
而不信发件人的 Content-Type；抓远程图片前检查**真正要连的 IP**（`net.Dialer.
Control`，见 `imagecache.go`）而不是域名——这一条字面意义上就是「不信任网络」。

**留痕**：导出会话**先写记录再交付字节**，记不上就不放行（§导出）。
...
```go
op := Operator{
    TenantID:   firstInt(md, "x-tenant-id"),
    EmployeeID: firstInt(md, "x-employee-id"),
    ...
}
```

**照单全收。** 没有签名、没有校验、没有 TLS。

后果写在 `docs/DEPLOY.md` 里，这里重复一次因为它是本节最重要的一句：
**任何能连上 9001 端口的人，不用密码就是所有租户的管理员。**

这在当初是合理的取舍——单机部署、内部端口全绑 `127.0.0.1`、网关是唯一入口，
在服务间再做一遍鉴权是重复劳动。**代价是全部赌注压在一道防线上**，而那道防线是
网络配置，不是代码。这也是部署清单第一条是「除了 443 什么都不要开」、并且特意
写明「别指望防火墙，Docker 的 iptables 规则排在 ufw 前面」的原因。

### 三个缺口（2026-08-10 复盘：两条已补）

| | 当时 | 现在 |
|---|---|---|
| **令牌吊销** | 没有，纯 bearer，唯一手段是换 `JWT_SECRET` 把全公司登出 | ✅ 每人一个时间戳，10 秒内生效，别人无感（见下 ②） |
| **令牌绑定** | 没有绑设备、绑 IP、绑指纹 | ⬜ 仍然没有，**但窗口从 24 小时缩到 12 小时闲置**（见下 ④） |
| **服务间身份** | 没有 | ✅ HMAC 签名（见下 ①） |

### 补的顺序（未决，按性价比）

1. ~~**服务间签名**~~ ✅ **2026-08-08 落地**（`pkg/grpcx/signing.go`）。
   HMAC-SHA256 覆盖 `版本|租户|员工|姓名|时间戳|方法名`，密钥 `INTERNAL_SIGNING_KEY`，
   11 个服务同一个值，**没有它服务起不来**——与 `MAIL_CRED_KEY` 同一条规矩：
   一个能从仓库里读到的签名密钥谁也证明不了。

   三个决定值得记下来：

   **签名认的是服务，不是人。** 邮箱同步、图片缓存、几个回填任务调用别的服务时
   没有登录态。只在有操作者时才签名，会让所有后台路径成为没设防的那一半——正是
   会被挑中的那一半。所以空身份也签。

   **方法名进签名。** 否则一个无害调用的签名可以被重放到破坏性调用上（同样的
   声明，换个方法）。它挡不住同一个调用在时间窗内被原样重放，那是小得多的事：
   重复一次已发生的动作，而不是凭空造一个。

   **拒绝的理由只进日志，不回给调用方。** 「你的时钟差太多」和「你的密钥不对」
   对排障的人是两回事，对探测的人是同一件事。

   实测：拿掉签名直连 9001、冒充员工 1 读员工名单 → `Unauthenticated`
   （此前返回全部员工）；11 个服务全部正常启动；后台任务被拒次数 **0**。
   这条集成测试留在 `services/iam/cmd/`，服务没起时它 **skip 而不是 pass**——
   一个因为服务没跑而变绿的测试，恰恰是本该拦住「这道防线被关掉」的那个。

   **它不是完整方案**：所有服务共用一个密钥，所以它证明「是我们中的一个」，
   不证明「是邮件服务」。绑定地址仍然是第一道防线，这条是第二道。
2. ~~**令牌吊销名单**~~ ✅ **2026-08-08 落地**
   （`services/gateway/internal/httpapi/revocation.go`）。每人一个时间戳：在它
   之前签发的令牌全部作废。不是逐个令牌的名单——令牌没有值得存的标识，而实际
   需要下达的指令永远是「他手上现在全部的」，从来不是「那一个会话」。

   **读的是网关内存里的快照，不是每次请求查 Redis。** 这一条是本设计的核心。
   每请求查 Redis 就必须回答「Redis 连不上怎么办」，而两个答案都很糟：
   放行 → 被吊销的会话恰好在基础设施已经出问题的时候复活；
   拒绝 → 一次 Redis 抖动把全公司登出，正是这个功能想避免的那种故障，只不过
   是自己造成的。快照有第三个答案：**继续执行最后已知的那份**。

   实测（真停掉 Redis 12 秒）：

   ```
   吊销前   A 200   B 200
   吊销后   A 401   B 401        两个都在吊销前签发
   重新登录  C 200                吊销不是封禁，禁止登录是「停用账号」的事
   停 Redis  A 401   C 200        继续执行，既没放行也没锁死
   日志     keeping the last known one, age=12s
   删掉记录  A 200                快照双向跟随，不是只进不出
   ```

   **「结束登录」和「标记离职」是两件事，分开又相连。** 分开：笔记本被偷不是
   辞职。相连：标记离职时顺带吊销——离职已经挡住了再次登录，但挡不住他手上那
   个还能用到明天早上的会话。一个是关门，一个是清场。

   吊销后的**同一秒**签发的令牌不受影响，这是对的：吊销结束的是已存在的会话，
   不阻止有凭据的人重新登录；阻止登录是停用账号的职责。
3. **服务间 TLS**——工作量最大，而且如果 ①② 做了、网络也确实关严，边际收益最小。
   真正需要它的时机是**服务开始跨机器部署**的那天，因为那时流量离开了本机回环。

我的倾向：**①②，③ 等到多机部署再说**。①② 都不依赖上云与否，现在做和以后做
成本一样，但现在做能在真实数据进来之前就位。

4. ~~**会话按最后活动算**~~ ✅ **2026-08-09 落地**
   （`pkg/authtoken` 的 `Renew`/`SignedInAt`，网关的 `renewIfHalfSpent`）。
   令牌用掉一半时，网关顺手签一张新的放进 `X-Renewed-Token` 响应头，前端拦截器
   换掉本地那张。`JWT_TTL` 的含义因此变了：从「距离登录多久」变成「**可以闲置
   多久**」，默认 24h → 12h。

   参照 ERPNext：它的 `session_expiry` 字段标题直接写着 *(idle timeout)*，默认
   170 小时。抄的是这个**行为**而不是那个数字——它是服务端会话，每个请求本来就
   要读写一次会话记录，续期对它是免费的；我们是无状态的，得自己签。

   **必须和 `auth_time` 一起做，否则会把 ② 架空。** 吊销比的是「签发时间 < 吊销
   时间」，而续期给新票一个新的签发时间。快照有最多 10 秒滞后，被吊销的人只要
   在这 10 秒里点一下，拿到的新票就落在吊销线之后——那 10 秒会从「最多晚 10 秒
   生效」变成「**永远不生效**」。所以票里另存一个 `auth_time`（最初登录时刻），
   续期时原样抄过去，吊销拿它来比。测试里那条是有牙的：把比较改回 `iat` 当场变红。

   续期放在验签和查吊销**之后**，不是之前——一个马上要被拒的会话不该在同一个
   响应里拿到新票。

### ⑤ 令牌存哪：localStorage → httpOnly cookie（待做）

**这是「令牌绑定」那一格剩下的那半。** 前四条都在缩短「被偷之后能用多久」，这
一条针对的是「**能不能被偷走**」。

**先把话说明白：签名对被偷的票完全无能为力。** 被偷的票签名是完美的——它本来
就是我们签的。签名防的是伪造和篡改，不是盗窃。所以这一格的防线全在别处。
...
|---|---|---|
| 网线上截获 | TLS（只开 443） | ✅ |
| 浏览器里偷（XSS） | 邮件正文服务端过滤 + 正文放在**不允许运行脚本**的隔离框 | ✅ 两层 |
| 人直接碰到那台机器 | 防不住。只能缩短窗口（④）+ 能踢掉（②） | ⚠️ 已缓解 |

**要做的：把令牌从 `localStorage` 挪进 httpOnly cookie。** JS 读不到，XSS 就偷
不走——这是唯一能把第二行从「两层防护」变成「结构上不可能」的改法。

代价和必须一起做的事：

- **CSRF 防护。** cookie 会被浏览器自动带上，所以「跨站发起的请求」重新成为问题。
  最小方案：`SameSite=Strict` + 状态变更请求带一个双提交 token。这不是可选项，
  换过去就必须一起做。
- **`X-Renewed-Token` 那条路要改。** 续期不再是响应头 + 前端写 localStorage，
  而是服务端 `Set-Cookie`。反而更简单，前端那段拦截器可以删掉。
- **跨源部署更麻烦。** cookie 的域和 `SameSite` 要对齐；现在同源，问题不大。
- **调试变难。** 令牌不再能从控制台读出来，本地排查要靠日志。

**为什么排在草稿自动保存后面：** 它挡的是 XSS，而 XSS 已经有两层在挡（而且是
服务端那层，绕不过去）；草稿丢失是**每天都会发生**的事。先做发生频率高的那个。

**一个反直觉的点，免得以后有人提议「干脆换成服务端会话」：** ERPNext 的 `sid`
也是 bearer——偷走那个 cookie 一样能冒充。服务端会话买到的**不是「防盗」，是
「好踢」**：删一行就当场失效，不需要 ② 那一整套时间戳和快照。我们用无状态换来
了网关不查库、能横着扩容，代价就是自己实现吊销。**两种模型都不解决盗窃。**
