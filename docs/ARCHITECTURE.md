...
**「发送」目前的语义（2026-07-27 记录）**：`SendQuotation` 只做状态流转与时间戳，
**系统不发邮件**——实际投递仍由销售自己完成。做成真正的发送需要三件事，排在阶段 6
与通知服务一起落地：

1. **报价单 PDF**：模板（抬头、条款、明细、有效期）+ 生成服务，产物存对象存储
2. **邮件通道**：export 往 outbox 写 `QuotationSent` 事件，通知服务消费后发信到
   `contact_email`；SMTP 故障不影响报价单本身的状态流转
3. **送达记录**：`sent_at` 之外补记投递结果（成功/退信），失败要能重发

**客户如何确认**：仍由销售在系统里录入客户答复（`RespondQuotation`）。
让客户自己点「接受」需要一个对外的客户门户（外部身份、链接安全、审计），
需求文档未提出，暂不做。
